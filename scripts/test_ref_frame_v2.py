...
# Load workflow from test script
with open("/tmp/test_ref_frame.py") as f:
    content = f.read()

# Extract workflow dict by re-running the script's workflow definition
exec_globals = {}
# Just submit directly with error handling
workflow = {
    "1": {
...
req = urllib.request.Request("http://127.0.0.1:8189/prompt", data=prompt_data, headers={"Content-Type": "application/json"})
try:
