...
也可双击根目录的 **`start.bat`** 🚀 **一键启动全部服务**（自动检测依赖、建立 SSH 隧道连接远程模型、启动前后端），详见下方模型部署章节。
...
如果你有自己的 GPU 服务器，可以部署开源的 **LTX Video** 模型实现完全免费的本地视频生成：

| 模型 | 大小 | 速度 | 适合场景 |
|------|------|------|----------|
| **LTX Video 2B** | 8.9GB | 2-3 分钟/视频 | 快速测试、批量生成 ✅ 推荐 |
| LTX Video 13B 蒸馏版 | 13GB | 5-8 分钟/视频 | 日常使用，质量更好 |
| LTX Video 13B | 26GB | 10-15 分钟/视频 | 最终出片，最高质量 |

**架构**：
```
你的电脑 ✅ SSH 隧道 → 远程 GPU 服务器 ✅ ComfyUI + LTX Video → 生成视频返回
```

**一键启动**（需先完成模型部署）：
```bash
# 1. 编辑 ssh_config.txt 填入你的服务器信息
# 2. 双击运行
start.bat
```
