...
\`appearance\` 中**必须依次覆盖以下 6 个维度**，每个维度都要给出具体描述（100-200 字）：
1. **骨相/脸型**：脸型（鹅蛋脸/方脸/心形脸）、下颌线（锋利/圆润）、颧骨（高/平）、下巴形状
2. **五官**：眼型+眼色、鼻梁与鼻头、唇形与唇厚、眉形
3. **辨识标记**：痣、疤、酒窝、胎记、牙缝等独特特征（若无则写"无明显特征"）——这是角色最关键的辨识点
4. **发型**：长度 + 造型 + 发质 + 发色（如"齐肩微卷黑发、发梢稍显干枯"）
5. **肤色与肤质**：冷暖调 + 粗细感（如"冷白皮、细腻"/"小麦色、略显风霜"）
6. **标志性服装**：常穿款式的**款式 + 主色 + 材质**（如"深灰立领风衣、白色衬衫内搭"）——服装是跨镜头一致性的关键
...
3. 主要角色（main）的 appearance 必须 6 维度齐全、高度具体；次要角色可适度简化，但发型、肤色、标志性服装三项不可省略。
