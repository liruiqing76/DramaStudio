...
Each element is a character object containing the above fields.`;
  ...
  return getDefaultPromptBody('character_extraction') + `\n- **风格要求**：${style}\n- **图片比例**：${imageRatio}\n输出格式：\n**重要：必须只返回纯JSON数组，不要包含任何markdown代码块、说明文字或其他内容。直接以 [ 开头，以 ] 结尾。**\n每个元素是一个角色对象，包含上述字段。`;
}
...
- segment_index must be sequential integers starting from 0; all shots in the same segment share the same index and title`;
  ...
  const _sbLocked = getLockedSuffix('storyboard_system') || '\n\n**重要：必须只返回纯JSON数组，不要包含任何markdown代码块、说明文字或其他内容。直接以 [ 开头，以 ] 结尾。**';
  if (_sbOverride) {
    return _sbOverride + _sbLocked;
  }
...
- camera_angle 机位：平视/仰视/俯视/侧面/背面
- camera_movement 运镜：从下方运镜清单中选一（不得留空）
- action 动作：**一个可执行的物理动作**，主语明确
- duration 时长：依据动作+对白估算，一般 2-8 秒；纯静态空镜 2-3 秒

**③ 单分镜单一语义**
- 一个分镜聚焦一个叙事节拍；若动作确实复杂，用"切至"式内部多切镜描述，不要写成一句无法执行的长句
- action 中出现的每个动作主体，都必须能在 characters 或画面元素中找到对应，禁止"一群人""众人"这类无法定妆的模糊主体

**④ 首尾帧可生成性校验（关键）**
每个镜头都会被生成"首帧（动作开始前的静态画面）"和"尾帧（动作结束后的静态画面）"：
- **首帧**：必须是**动作发生前的静止状态**，不得包含任何运动描述（禁止"正在跑"→ 应写"双脚蹬地、身体前倾、准备起跑的姿势"）
- **尾帧**：必须是**动作完成后的静止结果**，与首帧要有**明确可辨的视觉差异**（位置/姿态/表情/道具状态改变至少占其一）
- **禁止**生成"首尾帧几乎相同"的镜头；若某镜头首尾无变化，应合并或改写
- 首帧与尾帧应描述**同一空间、同一人物构成**，仅动作推进，避免跳轴或人物凭空增减

**⑤ 一致性约束**
- characters 必须与 action / dialogue 中实际出场人物严格一致
- 场景必须复用该场景已锁定的固定锚点（空间结构/材质/光线），只允许改变机位与光线强弱
- 角色外貌必须沿用其身份锚点，禁止在本环节重新发明外貌细节
...
- 【角色一致性】每个分镜的 characters 列表必须与该分镜 action/dialogue 中实际描写的人物严格一致，不得把在场但本镜头动作未涉及的角色列入

【输出前可拍性自检（逐条确认，任一不通过必须重写该分镜）】
□ 这个分镜是否只包含摄影机能拍到的人/物/光/动作/表情，无任何心理或抽象描写？
□ 五要素（景别/机位/运镜/动作/时长）是否全部明确、无空值、无模糊词？
□ 首帧（动作前静止态）与尾帧（动作后静止态）是否都能清晰成像，且二者存在可辨的视觉差异？
□ 本分镜是否与前后镜头在景别上形成了变化，避免了连续同景别？
□ characters 列表是否与本分镜实际出场人物完全对应，无多列也无漏列？`;
}
...
**duration时长**：${durationInstruction}。**必填，不得留空。**
...
- **首尾帧意识**：action 描述的动作要能自然拆出"开始前的静止态"和"结束后的静止态"，二者产生可见变化；若一个分镜想了半天也想不出首尾差异，请把它合并进相邻分镜，而不是硬凑一个废分镜。
- **场景复用**：location 与 atmosphere 必须复用该场景已锁定的固定锚点（空间结构/材质/主色/光照基调），同一场景在不同分镜里不得出现地板材质、墙面颜色、窗户位置等本体特征的前后矛盾。
- **角色复用**：characters 只列本分镜真正出场的角色；其外貌、服装一律沿用已定妆的身份锚点，不得在本环节新增或改动。
...
Each element: location, time, prompt (English image generation prompt for pure background).`;
  ...
  return getDefaultPromptBody('scene_extraction') + `\n5. **风格要求**：${s}\n   - **图片比例**：${imageRatio}\n\n【输出格式】\n**重要：必须只返回纯JSON数组，不要包含任何markdown代码块。直接以 [ 开头，以 ] 结尾。**\n每个元素包含：location（地点）, time（时间）, prompt（完整的中文图片生成提示词，纯背景，明确说明无人物）。`;
}
...
4. Each episode should have a clear beginning, development, and a hook or turning point at the end.${enNote}`;
  ...
  const base = _storyOverride || getDefaultPromptBody('story_expansion_system');
  // 占位符替换：默认正文来源于 skills/story_expansion_system.md，用 ${n} 表示集数
  const resolved = String(base).replace(/\$\{n\}/g, String(n));
  return resolved + jsonNote;
}
...
function getDefaultPromptBody(key) {
  const fromSkill = skillService.getDefaultBody(key);
  if (fromSkill) return fromSkill;
  switch (key) {
    case 'story_expansion_system':
      return `你是一位专攻**竖屏短剧**的资深编剧，深谙短剧「黄金3秒抓人、每分钟一个钩子」的创作规律。你的任务是根据用户提供的故事梗概，创作 \${n} 集完整的短剧剧本。
...
□ 是否未出现分镜格式与镜头编号？`;
    ...
    case 'storyboard_system':
      return '【角色】你是一位资深影视分镜师，精通罗伯特·麦基的镜头拆解理论，擅长构建情绪节奏。\n\n【任务】将小说剧本按**独立动作单元**拆解为分镜头方案。\n\n【分镜拆解原则】\n1. **动作单元划分**：每个镜头必须对应一个完整且独立的动作\n   - 一个动作 = 一个镜头（角色站起来、走过去、说一句话、做一个反应表情等）\n   - 禁止合并多个动作（站起+走过去应拆分为2个镜头）\n\n2. **景别标准**（根据叙事需要选择）：\n   - 大远景：环境、氛围营造\n   - 远景：全身动作、空间关系\n   - 中景：交互对话、情感交流\n   - 近景：细节展示、情绪表达\n   - 特写：关键道具、强烈情绪\n\n3. **运镜要求**：\n   - 固定镜头：稳定聚焦于一个主体\n   - 推镜：接近主体，增强紧张感\n   - 拉镜：扩大视野，交代环境\n   - 摇镜：水平移动摄像机，空间转换\n   - 跟镜：跟随主体移动\n   - 移镜：摄像机与主体同向移动\n\n4. **情绪与强度标记**：\n   - emotion：简短描述（兴奋、悲伤、紧张、愉快等）\n   - emotion_intensity：用箭头表示情绪等级\n     * 极强 ↑↑↑ (3)：情绪高峰、高度紧张\n     * 强 ↑↑ (2)：情绪明显波动\n     * 中 ↑ (1)：情绪有所变化\n     * 平稳 → (0)：情绪不变\n     * 弱 ↓ (-1)：情绪回落\n\n═══════════════════════════════════\n【5. 可拍性铁律（违反即不合格）】\n═══════════════════════════════════\n每一个分镜最终都会被 AI 直接生图并生视频，**任何"拍不出来"的描述都是废镜头**。请逐条自检：\n\n**① 必须可成像（拒绝不可视化描写）**\n- ❌ 禁止：心理活动、内心独白式的抽象描写（"他内心五味杂陈""她回忆起往事"）\n- ❌ 禁止：抽象概念（"气氛凝固了""时间仿佛静止""命运的齿轮开始转动"）\n- ✅ 必须：具体到摄影机能拍到的**人、物、光、动作、表情、空间关系**\n- 情绪必须**外化**为可见表征：攥紧的拳头、后仰的头、颤抖的嘴唇、避开的目光、摔在桌上的文件\n\n**② 五要素齐全（缺一不可）**\n每个镜头必须明确给出，不得留空、不得含糊：\n- shot_type 景别：大远景/远景/中景/近景/特写（五选一，不得写"适中""随机"）\n- angle 机位角度：平视/仰视/俯视/侧面/背面\n- movement 运镜方式：从运镜清单中选一（不得留空）\n- action 动作：**一个可执行的物理动作**，主语明确\n- duration 时长：依据动作+对白估算，一般 2-8 秒；纯静态空镜 2-3 秒\n\n**③ 单镜头单一语义**\n- 一个镜头聚焦一个叙事节拍；若动作确实复杂，用"切至"式内部多切镜描述，不要写成一句无法执行的长句\n- action 中出现的每个动作主体，都必须能在 characters 或画面元素中找到对应，禁止"一群人""众人"这类无法定妆的模糊主体\n\n**④ 首尾帧可生成性校验（关键）**\n每个镜头都会被生成"首帧（动作开始前的静态画面）"和"尾帧（动作结束后的静态画面）"：\n- **首帧**：必须是**动作发生前的静止状态**，不得包含任何运动描述（禁止"正在跑"→ 应写"双脚蹬地、身体前倾、准备起跑的姿势"）\n- **尾帧**：必须是**动作完成后的静止结果**，与首帧要有**明确可辨的视觉差异**（位置/姿态/表情/道具状态改变至少占其一）\n- **禁止**生成"首尾帧几乎相同"的镜头；若某镜头首尾无变化，应合并或改写\n- 首帧与尾帧应描述**同一空间、同一人物构成**，仅动作推进，避免跳轴或人物凭空增减\n\n**⑤ 景别节奏**\n- 不得连续 3 个镜头使用同一景别；对话戏优先中景/近景交替；情绪爆点用特写；环境交代用远景/大远景\n- 固定镜头占比不得超过 20%\n\n**⑥ 一致性约束**\n- characters 必须与 action / dialogue 中实际出场人物严格一致\n- 场景必须复用该场景已锁定的固定锚点（空间结构/材质/光线），只允许改变机位与光线强弱\n- 角色外貌必须沿用其身份锚点，禁止在本环节重新发明外貌细节\n\n【输出要求】\n1. 生成一个数组，每个元素是一个镜头，包含：\n   - shot_number：镜头号\n   - scene_description：场景（地点+时间，如"卧室内，早晨"）\n   - shot_type：景别（大远景/远景/中景/近景/特写）\n   - camera_angle：机位角度（平视/仰视/俯视/侧面/背面）\n   - camera_movement：运镜方式（static/推镜push/拉镜pull/横摇pan/纵摇tilt/跟镜tracking/升镜crane_up/降镜crane_dn/环绕orbit/手持handheld/变焦zoom/旋转roll/甩镜whip_pan/螺旋spiral/希区柯克hitchcock_zoom/子弹时间bullet_time/荷兰角dutch_angle_move/推轨复合dolly_track/升格环绕slowmo_orbit）——**强制动态优先，固定镜头不得超过20%**\n   - action：动作描述\n   - result：动作完成后的画面结果\n   - dialogue：角色对话或旁白（如有）\n   - emotion：当前情绪\n   - emotion_intensity：情绪强度等级（3/2/1/0/-1）';

    case 'character_extraction':
      // 注意：skills/character_extraction.md 存在时上面已提前 return；此处为无 skill 文件时的内联兜底
      return `你是一个专业的角色定妆师与视觉分析师，擅长从剧本中提取角色信息，并输出**可直接用于 AI 生图且跨镜头可复用**的一致身份锚点。
...
\`appearance\` 字段将被下游用于提炼 6 层视觉锚点（骨相/五官/辨识标记/色值/皮肤/发型），
...
4. **辨识标记**：痣、疤、酒窝、胎记、牙缝等独特特征（若无则写"无明显特征"）——这是角色最关键的辨识点
...
2. 对每个角色，提取以下字段（全部用中文填写）：
   ...
   - appearance: 按上述 6 维度撰写的一致身份锚点描述（100-200 字，**不含任何场景、环境、动作、情绪信息**）
   - description: 背景故事和角色关系（50-100 字）
3. 主要角色（main）的 appearance 必须 7 维度齐全、高度具体；次要角色可适度简化，但基础信息（性别/年龄/身高/体型）、发型、肤色、标志性服装四项不可省略。
...
function getLockedSuffix(key) {
  const fromSkill = skillService.getLockedSuffix(key);
...
- **年龄与体型必须明确**：必须在【基础设定】中写出具体年龄段（如"20-25岁"）和身高体型（如"170cm/标准修长"），禁止使用"青年""中年"等模糊词；所有面板的体型比例必须与此一致
...
【基础设定】
人物基础: 性别，具体年龄段（如"20-25岁"，禁止"青年""中年"等模糊词），身高（如"170cm"），体型（如"标准修长""微胖""健壮"），肤色
五官: 眉形，眼型，瞳色，鼻型，唇形
...
5. **角色外貌描述规则（Wan2.1适配）**：
   - 角色外貌**必须完整描述**固定身份特征 + 服装风格（脸型、五官、发型、肤色、服装颜色/款式、配饰等）。
   - **服装描述是角色一致性的关键**，Wan2.1需要明确的服装描述才能保持角色视觉一致。
   - 服装风格应与角色参考图/锚点保持一致，不要随意改变角色穿着。
   - 如有角色参考图(ASSETS)，服装应与参考图一致，但仍需在prompt中用文字描述以强化。
...
/**
 * 单帧分镜图 VLM 质检 prompt
 * 用视觉语言模型评估生成的分镜图是否合格，输出 JSON 评分+问题列表+优化建议
 * 移植自 VideoClaw prompts/reference/eval_first_frame_zh.txt
 */
function getVlmFrameEvalPrompt() {
  return `你是一个专业的视频分镜参考图评估师。请评估生成的图片是否适合作为该分镜的视频首帧/参考图。
...
请先判断【硬性失败项】。只要命中任意硬性失败项，is_acceptable 必须为 false，即使图片美观也不能通过。

【硬性失败项】
1. 核心角色数量错误：分镜需要出现的主要人物明显多了或少了。
2. 核心角色不可辨认：主要人物脸部/身体严重崩坏、被遮挡、被裁切，或无法看清是谁。
3. 角色资产明显不一致：主要人物的性别、年龄段、核心服装、核心发型与角色描述明显不符。
4. 场景地点错误：图片环境明显不是分镜要求的地点。
5. 人物状态/人物关系完全错误：图片没有体现片段开始前的人物状态、位置关系、视线方向或核心情绪，且无法作为该片段的视频起点。
6. 景别/构图不可用：主体严重偏出画面、构图严重倾斜、人物关系看不清，或不适合做视频首帧。
7. 人物与场景互动错误：人物不能与桌子、椅子、门、墙、车辆、道具等发生明显穿模、嵌入、悬浮、错位接触或不合理遮挡。
8. 出现文字、水印、标签、标注线、说明文字或明显UI元素。
9. 图片严重模糊、过曝、过暗、低清或畸变，无法作为视频首帧。

若没有硬性失败项，再评估【软性维度】并给出1-10分。软性问题可以扣分，但不应单独导致不合格。

【软性维度】
1. 人物静态状态匹配：图片体现片段开始前人物的站姿、坐姿、表情、情绪和准备状态。
2. 人物站位与交互：人物之间的位置、朝向、视线关系是否清楚。
3. 角色外观一致性：发型、服装、年龄气质是否大体贴合角色资产。
4. 场景一致性：地点、空间布局、关键物件是否大体贴合场景资产。
5. 镜头语言：景别、机位、平视/俯视/仰视等是否符合分镜视觉描述。
6. 视频可用性：主体清楚，人物状态和互动关系明确，适合后续 i2v。
7. 光线氛围：光线、色调、时间感是否服务剧情。
8. 图像质量：清晰度、细节、手脸身体质量是否足够。

【合格标准】
- 如果 hard_failures 非空：is_acceptable=false，score 最高不得超过6。
- 如果 hard_failures 为空且 score >= 7：is_acceptable=true。
- 如果 hard_failures 为空但 score < 7：is_acceptable=false。
- "人物穿过桌子/椅子/墙/道具"属于硬性失败项，不得因为整体画面好看而放行。
- 不要因为"没有完全复刻所有台词细节""光线不够电影感""背景细节略少"等软问题直接判失败。

如果需要重新生成，请提供 suggested_prompt。优化提示词时：
- 不要改变角色的核心外貌描述，如发型、服装、年龄、性别。
- 可以加强镜头、站位、人物视线、人物状态、构图、光线和场景元素。
- 保持与分镜剧情和视觉描述一致。

请输出纯JSON格式（不要用\`\`\`包裹）：
{
  "score": 1-10,
  "hard_failures": ["硬性失败项1", "硬性失败项2"],
  "soft_issues": ["软性问题1", "软性问题2"],
  "issues": ["合并后的主要问题列表"],
  "is_acceptable": true/false,
  "suggestion": "如果需要重新生成，说明最应该修正什么；如果可接受则为空字符串",
  "suggested_prompt": "优化后的提示词；如果不需要重新生成则为空字符串"
}`;
}

/**
 * VLM 多版本择优 prompt
 * 当重生成产生多个版本时，用 VLM 从中选最好的一张
 * 移植自 VideoClaw prompts/character/eval_select_best_zh.txt
 */
function getVlmSelectBestPrompt() {
  return `你是一个专业的AI图像质量评估师。请从以下{num_images}张分镜参考图中选择最好的一张。
...
1. 角色一致性：角色外观是否与描述一致（发型、服装、配饰）
2. 面部特征：面部特征是否清晰、符合描述
3. 身体比例：身体比例是否正常、协调
4. 风格一致性：整体风格是否符合设定风格
5. 细节完整：细节是否丰富，无明显缺失
6. 色彩还原：色彩是否与描述一致
7. 画面质量：清晰度良好，无模糊、无畸变
8. 构图质量：构图是否均衡、美观
9. 场景匹配：场景是否与描述一致
...
/**
 * 站位连续性检查 prompt
 * 检查一集内人物站位、朝向和空间关系是否连贯，输出 patches 修正
 * 移植自 VideoClaw prompts/storyboard/staging_continuity_zh.txt
 */
function getStagingContinuityPrompt() {
  return `你是影视分镜连续性检查员。请检查第 {episode_number} 集《{episode_title}》中人物站位、朝向和空间关系是否连贯，并在不改变分镜结构的前提下修正不连贯的分镜描述。
...
检查规则：
1. 以场景为单位检查连续性；场景切换后，站位连续性可以重新建立。
2. 同一场景内，如果没有明确入场、离场、换座、走到另一侧、绕到桌后等移动动作，同一人物的相对位置、朝向和与桌椅门墙的空间关系应保持一致。
3. 有明确移动动作时，后续分镜必须承接移动结果。例如"走到门口"之后，人物应在门口附近；"坐下"之后，人物应坐在椅子上；"站起"之后，不应继续描述为坐着。
4. 多人对话/对峙中，人物的左右关系、隔桌关系、门边/桌旁/主位等关键站位不要无故跳变。
5. 如果前后分镜都在同一桌子/会议桌/办公桌旁，要特别检查人物是否突然穿过桌子、站到桌子内部、身体嵌入椅背或墙面。修正时必须写清合理空间关系，例如"站在桌侧""坐在椅子上""隔着会议桌相对而立"。
6. 每个片段第一个分镜如果涉及多个人物，仍需保持"平视、能同时拍到片段中的所有人物、站位清楚"。
7. 只修正站位、朝向、视线、机位和人物与场景物体的空间关系；不要改剧情，不要改台词，不要新增或删除分镜。
8. 不要在 content 中写"原文："。
9. 必须严格遵守每个片段的 scene_context 和 scene_space。如果 scene_space 是"外"，不得把镜头修成"室内、内景、房间内"等；如果 scene_space 是"内"，不得把镜头修成"室外、户外、露天"等。
10. 如果没有发现不连贯问题，patches 输出空数组。
...
// 重新导出：包含原有所有函数 + 新增 VLM 函数
// 注意：上方的 module.exports 已在 getRegenerateLayoutDescriptionPrompt 后闭合，
// 这里用 Object.assign 追加新函数，避免重复声明
const _origExports = module.exports;
Object.assign(_origExports, {
  getVlmFrameEvalPrompt,
  getVlmSelectBestPrompt,
  getStagingContinuityPrompt,
});
