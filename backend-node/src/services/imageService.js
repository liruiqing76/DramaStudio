      ...
      if (sb) {
        ...
        if (charListParsed && charListParsed.length) {
          for (const item of charListParsed) {
            ...
            const c = db.prepare('SELECT image_url, local_path, name, default_outfit_id FROM characters WHERE id = ? AND deleted_at IS NULL').get(Number(cid));
            if (!c) continue;
            // 衣橱优先：(1) 分镜 characters 条目显式指定的 outfit_id；(2) 角色默认造型。用对应造型正面图作参考
            let charRef = null;
            let isPanel = false;
            let isOutfitView = false;
            const explicitOutfitId = (typeof item === 'object' && item != null) ? Number(item.outfit_id) : NaN;
            const outfitId = Number.isFinite(explicitOutfitId) && explicitOutfitId > 0
              ? explicitOutfitId
              : (c.default_outfit_id != null ? Number(c.default_outfit_id) : null);
            if (outfitId != null && outfitId > 0) {
              try {
                const outfit = db.prepare(
                  'SELECT front_image_path FROM character_outfits WHERE id = ? AND deleted_at IS NULL'
                ).get(outfitId);
                if (outfit && (outfit.front_image_path)) {
                  charRef = outfit.front_image_path;
                  isOutfitView = true;
                }
              } catch (_) {}
            }
            if (!charRef) {
              ...
              charRef = c.local_path || c.image_url;
              if (!charRef) {
            ...
            if (charRef) {
              refs.push(charRef);
              refLabels.push(`Image ${refs.length}: character appearance reference for "${c.name || 'character'}"${isOutfitView ? ' (default outfit front view)' : isPanel ? ' (front full-body view from history panel)' : ' (current character image)'}`);
            }
          }
        }
          ...
          if (explicitDramaCharIds !== null && explicitDramaCharIds.length === 0) {
            ...
            for (const dChar of dramaChars) {
              ...
              const dCharRow = db.prepare(
                'SELECT image_url, local_path, default_outfit_id FROM characters WHERE id = ? AND deleted_at IS NULL'
              ).get(Number(dChar.id));
              // 衣橱优先：默认造型正面图 > characters 主图 > 历史面板
              let charRef = null;
              let isPanel = false;
              let isOutfitView = false;
              if (dCharRow?.default_outfit_id != null) {
                try {
                  const outfit = db.prepare(
                    'SELECT front_image_path FROM character_outfits WHERE id = ? AND deleted_at IS NULL'
                  ).get(Number(dCharRow.default_outfit_id));
                  if (outfit && outfit.front_image_path) {
                    charRef = outfit.front_image_path;
                    isOutfitView = true;
                  }
              ...
              if (!charRef) charRef = dCharRow?.local_path || dCharRow?.image_url;
              if (!charRef) {
                const charPanel = db.prepare(
                  `SELECT local_path, image_url FROM image_generations
                   WHERE character_id = ? AND frame_type = 'quad_panel_1' AND status = 'completed'
              ...
              if (charRef && !imageClient.refListHasCanonical(refs, charRef)) {
                refs.push(charRef);
                refLabels.push(`Image ${refs.length}: character appearance reference for "${dChar.name}"${isOutfitView ? ' (default outfit front view)' : isPanel ? ' (front full-body view from history panel)' : ' (character image)'}`);
