...
async function processPropExtraction(db, log, taskId, episodeId) {
  ...
  try {
    response = await aiClient.generateText(db, log, 'text', prompt, systemPrompt, {
      ...
      // agnes-3.0-flash 是推理模型：正式输出前会先生成大量 reasoning_content，
      // 小 max_tokens 会被思考过程耗尽导致 content 为空或 JSON 截断。
      // 必须给足 token 预算，保证最终 JSON 数组完整输出。
      // 注意：不要用 json_mode=true —— 该模式下模型倾向输出单个对象而非数组。
      max_tokens: 8000,
      temperature: 0.3,
    });
  } catch (err) {
