...
const storyTemplateMatcher = require('./storyTemplateMatcher');
...
async function generateStory(db, log, body) {
  ...
  // 题材模板注入：让剧本创作有骨架可依，而不是每次「随意写一个剧本」。
  // 匹配失败/模板异常时静默降级，绝不影响主流程。
  try {
    const template = storyTemplateMatcher.matchTemplate(premise, style, type);
    if (template) {
      const guidance = storyTemplateMatcher.buildTemplateGuidance(template);
      if (guidance) {
        systemPrompt += '\n' + guidance;
        log && log.info && log.info('Story template matched', {
          template_id: template.id,
          template_name: template.name,
        });
      }
    } else {
      log && log.info && log.info('Story template not matched, using generic prompts', {});
    }
  } catch (e) {
    log && log.warn && log.warn('Story template matching failed, fallback to generic', { error: e.message });
  }
