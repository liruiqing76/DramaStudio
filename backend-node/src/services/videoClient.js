...
function inferVideoProtocol(provider) {
  ...
  // MiniMax（海螺）必须显式识别：其官方接口与 OpenAI 兼容格式不同，
  // 若落到默认 openai 分支会请求错端点。前端 providerProtocolMap 亦映射为 'minimax'。
...
function resolveVideoProtocol(config, modelHint) {
  ...
  // config.model 可能是数组（DB 多模型配置）、字符串或空 —— 先归一化再比较，
  // 否则 String(['a','b']) 会得到 "a,b"，且在旧代码里曾出现 .split is not a function 崩溃。
  const rawConfigModel = Array.isArray(config.model) ? (config.model[0] || '') : (config.model || '');
  const configModel = String(rawConfigModel).toLowerCase();
  const modelLower = String(modelHint || '').toLowerCase();
  // Agnes AI 检测：即使 api_protocol 显式设为 openai，如果 base_url 或 model 指向 Agnes 则自动切换
  if (protocol === 'openai' && (baseLower.includes('agnes') || configModel.includes('agnes') || modelLower.includes('agnes'))) {
    protocol = 'agnes';
  }
...
function buildQueryUrl(config, taskId) {
  ...
  const base = (config.base_url || '').replace(/\/$/, '');
  let defaultEp;
  if (proto === 'agnes') defaultEp = null; // Agnes 使用特殊查询格式
  ...
  // Agnes 查询：官方推荐 GET {origin}/agnesapi?video_id={taskId}&model_name={model}（适用于所有模式）
  if (proto === 'agnes') {
    const origin = base.replace(/\/v\d+\/?$/, '').replace(/\/$/, '');
    // config.model 可能是数组（DB 中多模型配置）、字符串或空，统一归一化为首个模型名
    const rawModel = Array.isArray(config.model) ? (config.model[0] || 'agnes-video-2.5-flash') : (config.model || 'agnes-video-2.5-flash');
    const modelName = String(rawModel).split('/').pop();
    return origin + '/agnesapi?video_id=' + encodeURIComponent(taskId) + '&model_name=' + encodeURIComponent(modelName);
  }
...
async function callDashScopeVideoApi(config, log, opts) {
  ...
  if (model === 'wan2.2-kf2v-flash') {
    ...
    if (!firstUrl || !lastUrl) {
      return { error: 'wan2.2-kf2v-flash 需要同时提供首帧与尾帧图片' };
    }
    body = {
      model,
    ...
    if (!imgUrl) return { error: 'wan2.6-i2v-flash 需要提供首帧图片' };
    body = {
      model,
    ...
    if (refs.length === 0) return { error: 'wanx2.1-vace-plus 需要提供参考图（最多 3 张）' };
    body = {
      model,
    ...
    const refs = rawRefs.map(toImageInput).filter(Boolean);
    if (refs.length === 0) return { error: 'wan2.6-r2v-flash 需要提供参考图（最多 5 张）' };
    body = {
      model,
      input: { prompt: prompt || '', reference_urls: refs },
      parameters: { prompt_extend: true },
    };
    } else if (model.startsWith('wan3.0')) {
    // ── 万相 3.0（DashScope 官方）：支持 media 数组，reference_image / first_frame / last_frame ──
    url = base + DASHSCOPE_VIDEO_GENERATION;
    const media = [];
    const firstRaw3 = (first_frame_url && first_frame_url.trim()) || (image_url && image_url.trim());
    const lastRaw3 = (last_frame_url && last_frame_url.trim());
    const firstUrl3 = firstRaw3 ? toImageInput(firstRaw3) : null;
    const lastUrl3 = lastRaw3 ? toImageInput(lastRaw3) : null;
    if (firstUrl3) media.push({ type: 'first_frame', url: firstUrl3 });
    if (lastUrl3) media.push({ type: 'last_frame', url: lastUrl3 });
    if (media.length === 0 && Array.isArray(reference_urls)) {
      for (const u of reference_urls.filter(Boolean).slice(0, 10)) {
        const pub = toImageInput(u);
        if (pub) media.push({ type: 'reference_image', url: pub });
      }
    }
    body = {
      model,
      input: { prompt: prompt || '' },
      parameters: { resolution: '720P', ratio: '9:16', duration: dur, audio: true, prompt_extend: true },
    };
  ...
  const { withRateLimitRetry } = require('../utils/rateLimitRetry');
  const res = await withRateLimitRetry(async () => {
    ...
    return { status: attemptRes.status, text: attemptRaw };
  }, { log, tag: `dashscope-video:${model}`, maxRetries: 3, baseDelayMs: 5000 });
  const raw = res ? res.text : '';
  const respStatus = res ? res.status : 0;
  if (!res || respStatus < 200 || respStatus >= 300) {
    let errMsg = '视频生成请求失败: ' + respStatus;
    try {
      const errJson = JSON.parse(raw);
    ...
    log.error('DashScope video create failed', { status: respStatus, body: raw.slice(0, 300), video_gen_id });
    return { error: errMsg };
  }
  let data;
  try {
  ...
  return { error: 'DashScope 响应缺少 task_id 与 video_url' };
}

/**
 * Agnes reference 模式提示词增强 — 生成官方 <Picture N> 引用前缀。
 *
 * 官方文档（agnes-video-25「提示词建议」）：
 *   "在 reference 模式中，应在提示词里明确写出素材占位符及其用途，
 *    例如『以 <Picture 1> 为角色参考，并跟随 <Audio 1> 的节奏』。
 *    这比只上传素材但不解释用途更容易获得可控结果。"
 *   「编号规则」：images 数组从 1 开始编号，第 N 张即 <Picture N>。
 *
 * 原来的实现只把 images 数组丢给上游、prompt 里从不提占位符，
 * 模型无法分辨哪张是角色定妆图、哪张是场景图，角色一致性靠运气。
 *
 * @param {Array<{kind:string,name:string}>} items 与 images 数组同序的素材语义
 * @returns {string} 形如「以 <Picture 1> 中的角色"林风"外貌与服装为参考，以 <Picture 2> 的场景环境为背景。」的前缀
 */
function buildAgnesPictureReferencePrefix(items, log, video_gen_id) {
  ...
  items.forEach((it, idx) => {
    ...
    if (it && it.kind === 'character') {
      parts.push('以 ' + tag + ' 中的角色' + (nm ? '「' + nm + '」' : '') + '外貌、发型与服装为参考，保持角色一致性');
    } else if (it && it.kind === 'scene') {
      parts.push('以 ' + tag + ' 的场景环境、光影与美术风格为背景');
    } else {
      parts.push('以 ' + tag + ' 为视觉参考');
    }
  });
  const prefix = parts.join('；') + '。';
  log.info('[Agnes reference] 已生成 <Picture N> 引用前缀', {
    video_gen_id, ref_count: items.length, prefix: prefix.slice(0, 200),
  });
  return prefix;
}
...
function adaptPromptForAgnes(rawPrompt, mode, log, video_gen_id) {
  ...
  if (mode === 'keyframe') {
    p = p.replace(/(?:，\s*)?(?:photorealistic|ultra-detailed|8k\s*uhd|sharp\s*focus|natural\s*lighting|real\s*skin\s*texture|hyperrealism|professional\s*photography|RAW\s*photo)[\s，,]*/gi, ' ');
  }
  // 清理噪音
  ...
  if (p.length > maxLen) {
    ...
    if (lastStop > maxLen * 0.5) p = p.slice(0, lastStop + 1);
  }
  log.info('[Agnes prompt适配]', { video_gen_id, mode, before_len: rawPrompt.length, after_len: p.length });
  return p;
}

/**
 * Agnes 首尾帧差异度检测 — 32x32 灰度均值绝对差
 * >0.22 差异过大（形变扭曲），<0.02 几乎相同（定格无意义）→ 均应丢弃尾帧
 */
async function agnesFrameDiffRatio(bufA, bufB, log, video_gen_id) {
  if (!sharp) return null;
  try {
    const [a, b] = await Promise.all([
      sharp(bufA).resize(32, 32).grayscale().raw().toBuffer(),
      sharp(bufB).resize(32, 32).grayscale().raw().toBuffer(),
    ]);
    let sum = 0;
    for (let i = 0; i < a.length; i++) sum += Math.abs(a[i] - b[i]);
    const diff = sum / (a.length * 255);
    log.info('[Agnes 首尾帧差异度]', { video_gen_id, diff: Number(diff).toFixed(4) });
    return diff;
  } catch (e) {
    log.warn('[Agnes 首尾帧差异度检测失败]', { video_gen_id, error: e.message });
    return null;
  }
}
 ...
 * reference 和 keyframe 互斥；角色一致性优先用 reference 模式
 */
async function callAgnesVideoApi(config, log, opts) {
  ...
  const AGNES_MODELS = ['agnes-video-2.5-flash', 'agnes-video-2.5'];
  const model = preferredModel || 'agnes-video-2.5-flash';
  const base = (config.base_url || 'https://api.agnes-ai.cn/v1').replace(/\/$/, '');
  let ep = config.endpoint || '/videos';
  if (!ep.startsWith('/')) ep = '/' + ep;
  const url = base + ep;

  // 媒体转公网 URL（已是公网直传；本地/localhost → 优先 Data URI Base64，必要时才走图床）
  //
  // 官方文档：输入媒体「应可由 Agnes AI 服务公开访问」。本地图有两条路可走：
  //   (1) Data URI Base64 —— 无外部依赖，Agnes 图片接口已明确支持，视频接口同样可用；
  //   (2) 第三方图床 —— 需要 image_proxy.use_for_video 开启，且图床不稳定会成批失败
  //       （历史报错：`port 5679 is not allowed`、`download input image failed: HTTP 404`）。
  // 因此默认优先 base64；仅当配置显式要求图床时才回退到上传图床。
  const toPublicMedia = async (val, tag) => {
    const s = String(val || '').trim();
    if (!s) return null;
    if (/^https?:\/\//i.test(s) && !/localhost|127\.0\.0\.1/i.test(s)) return { url: s, buffer: null };
    let buffer = null;
    let mime = 'image/jpeg';
    try {
      const resolved = resolveImageToBuffer(s, files_base_url, storage_local_path);
      if (resolved && resolved.buffer) {
        buffer = resolved.buffer;
        mime = resolved.mime || 'image/jpeg';
      }
    } catch (e) {
      log.warn('[Agnes Video] 本地图片读取为 Buffer 失败', { video_gen_id, tag, src: s.slice(0, 120), error: e.message });
    }
    const preferBase64 = !useImageProxyForVideo();
    if (preferBase64 && buffer) {
      // 官方支持 Data URI Base64，绕开图床，杜绝 localhost 泄漏与图床抖动
      return { url: 'data:' + mime + ';base64,' + buffer.toString('base64'), buffer };
    }
    if (preferBase64 && !buffer) {
      // 配置为优先 base64 却拿不到本地文件 → 即将回退图床（不稳定路径），必须显式告警
      log.warn('[Agnes Video] 优先 base64 但无法读取本地图片，将回退第三方图床（不稳定）', {
        video_gen_id, tag, src: s.slice(0, 120),
        hint: '检查 storage_local_path 与文件是否存在；或路径是否为 /static/ 前缀',
      });
    }
    if (buffer) {
      try {
        const uploaded = await uploadToImageProxy(buffer, mime, log, tag);
        if (uploaded) return { url: uploaded, buffer };
      } catch (_) {}
    }
    try {
      const up2 = await uploadLocalImageToProxy(storage_local_path, s, log, tag);
      if (up2) return { url: up2, buffer };
    } catch (_) {}
    log.warn('[Agnes Video] 媒体无法转为公网 URL', { video_gen_id, tag, src: s.slice(0, 120) });
    return null;
  };

  const firstMedia = await toPublicMedia(first_frame_url || image_url, 'first_frame');
  const lastMedia = await toPublicMedia(last_frame_url, 'last_frame');
  const refPublic = [];
  for (const u of (Array.isArray(reference_urls) ? reference_urls : []).slice(0, 5)) {
    const m = await toPublicMedia(u, 'reference_image');
    if (m) refPublic.push(m.url);
  }

  // 模式判定：reference（角色定妆照/场景图）> keyframe（首帧锁构图）> text
  //
  // ⚠️ 官方硬约束（实测 HTTP 400）：首尾帧素材与参考素材**不能同时使用**
  //    报错原文："首尾帧素材与参考素材不能同时使用"（param: mode）
  //    对应官方文档「生成模式规则」表：
  //      mode=reference → 不允许 first_frame / last_frame / videos，必须有 images 或 audios
  //      mode=keyframe  → 不允许 images / audios / videos，必须有 first_frame 或 last_frame
  //      mode=text      → 不允许任何媒体字段
  ...
  let firstPublic = firstMedia ? firstMedia.url : null;
  let lastPublic = lastMedia ? lastMedia.url : null;

  if (refPublic.length > 0) {
    // reference 模式：角色定妆照 + 场景图（官方上限 5 张），据此保持角色一致性。
    // 该模式下禁止传首尾帧，因此这里不写入 first_frame/last_frame。
    body.mode = 'reference';
    body.images = refPublic.slice(0, 5);
    if (firstPublic || lastPublic) {
      log.info('[Agnes Video] reference 模式已丢弃首尾帧（官方禁止与参考素材同时使用）', {
        video_gen_id,
        dropped_first_frame: !!firstPublic,
        dropped_last_frame: !!lastPublic,
        images_count: body.images.length,
      });
    }
  } else if (firstPublic || lastPublic) {
    // keyframe 模式：无参考图时用首帧锁构图
    body.mode = 'keyframe';
    if (firstPublic) body.first_frame = firstPublic;
    if (lastPublic) body.last_frame = lastPublic;
    if (firstMedia && firstMedia.buffer && lastMedia && lastMedia.buffer) {
      const diff = await agnesFrameDiffRatio(firstMedia.buffer, lastMedia.buffer, log, video_gen_id);
      // 官方文档建议尾帧有效性阈值 0.5：差异 > 0.5 视为形变扭曲（不可用作尾帧），
      // < 0.02 视为几乎定格（作为尾帧无意义）。原实现用 0.22，会把大量合法的
      // "动作型"尾帧误判为形变而静默丢弃，使 keyframe 退化为近似 text 模式。
      const AGNES_LAST_FRAME_MAX_DIFF = 0.5;
      const AGNES_LAST_FRAME_MIN_DIFF = 0.02;
      if (diff != null && (diff > AGNES_LAST_FRAME_MAX_DIFF || diff < AGNES_LAST_FRAME_MIN_DIFF)) {
        log.warn('[Agnes Video] 丢弃尾帧（差异度 ' + Number(diff).toFixed(3) + '，阈值 ' + AGNES_LAST_FRAME_MAX_DIFF + '/' + AGNES_LAST_FRAME_MIN_DIFF + '）', { video_gen_id });
        lastPublic = null;
        // 已加入 body 时同步移除
  ...
  // text 模式：无任何媒体字段（body 保持初始的 mode='text'）
  body.aspect_ratio = ar;
  // 先清洗原始 prompt（剥离分镜表标签/摄影括注），再在 reference 模式前置 <Picture N> 引用。
  // 顺序很重要：若先加前缀再清洗，「场景/风格:」等标签规则可能误伤前缀文本。
  const cleanedPrompt = adaptPromptForAgnes(prompt, body.mode, log, video_gen_id) || (prompt || '');
  if (body.mode === 'reference') {
    const picPrefix = buildAgnesPictureReferencePrefix(agnes_ref_items, log, video_gen_id);
    body.prompt = picPrefix ? picPrefix + cleanedPrompt : cleanedPrompt;
  } else {
  ...
  const { withRateLimitRetry } = require('../utils/rateLimitRetry');
  const { acquireAgnesSlot, applyAgnesCooldown } = require('../utils/agnesRateLimiter');
  let agnes429Streak = 0;
  const res = await withRateLimitRetry(async () => {
    // 官方 tokenplan：视频模型 TokenPlan 实际 RPM = 5（default 仅 1），
    // 创建任务前先按官方 RPM 主动排队，避免 503/429（历史报错 `Agnes视频请求失败: 503` ×9）
    await acquireAgnesSlot({ kind: 'video', log, tag: `agnes-video:${model}` });
    const attemptRes = await fetch(url, {
      method: 'POST',
    ...
    if (attemptRes.status === 429) {
      agnes429Streak += 1;
      applyAgnesCooldown(agnes429Streak, { log, tag: `agnes-video:${model}` });
    }
    return { status: attemptRes.status, text: attemptRaw };
  }, { log, tag: `agnes-video:${model}`, maxRetries: 4, baseDelayMs: 60000, maxDelayMs: 180000 });
  const raw = res ? res.text : '';
  const respStatus = res ? res.status : 0;
  log.info('Agnes视频原始响应', { video_gen_id, status: respStatus, raw: raw.slice(0, 1000) });
  if (!res || respStatus < 200 || respStatus >= 300) {
    let errMsg = 'Agnes视频请求失败: ' + respStatus;
    try { const j = JSON.parse(raw); if (j.detail) errMsg += ' - ' + j.detail; } catch (_) {}
    return { error: errMsg };
  }
  try {
    const j = JSON.parse(raw);
    const taskId = j.video_id || j.task_id || j.id;
    if (taskId && (j.status === 'queued' || j.status === 'processing' || !j.status)) {
      return { task_id: taskId, status: 'PENDING' };
    }
    if (j.metadata && j.metadata.url) return { video_url: j.metadata.url };
    if (j.video_url) return { video_url: j.video_url };
    return { task_id: taskId, status: 'PENDING' };
  } catch (e) {
    return { error: 'Agnes响应解析失败: ' + e.message };
  }
}
 ...
 * content: [{type:'text',text:prompt}, {type:'image_url',image_url:{url},role:'first_frame'}, ...]
 * 异步轮询: GET {base}/v1/video_generation/{task_id}
 */
async function callMiniMaxVideoApi(config, log, opts) {
  const {
  ...
  // 默认模型：MiniMax H3（新一代）。历史 Hailuo-02 / 2.3 仍可传入使用。
  const model = preferredModel || 'MiniMax-Hailuo-03';
  const base = (config.base_url || 'https://api.minimax.chat').replace(/\/$/, '');
  const apiKey = config.api_key || '';

  const toPublicUrl = async (val, tag) => {
    const s = String(val || '').trim();
    if (!s) return null;
    ...
    try {
      const resolved = resolveImageToBuffer(s, files_base_url, storage_local_path);
      if (resolved && resolved.buffer) {
        const uploaded = await uploadToImageProxy(resolved.buffer, resolved.mime || 'image/jpeg', log, tag);
  ...
  // first_frame
  const firstUrl = await toPublicUrl(first_frame_url || image_url, 'first_frame');
  if (firstUrl) content.push({ type: 'image_url', image_url: { url: firstUrl }, role: 'first_frame' });
  // reference images (角色定妆照)
  if (Array.isArray(reference_urls)) {
    for (const u of reference_urls.filter(Boolean).slice(0, 9)) {
      const pub = await toPublicUrl(u, 'reference_image');
      if (pub) content.push({ type: 'image_url', image_url: { url: pub }, role: 'reference_image' });
    }
  }

  if (!content.some(c => c.type === 'text') && content.length === 0) {
    return { error: 'MiniMax 需要 prompt 或参考图' };
  }

  const body = { model, content };
  if (duration) body.duration = Math.min(10, Math.max(1, Math.round(Number(duration))));
  if (aspect_ratio) body.aspect_ratio = aspect_ratio;

  log.info('MiniMax视频请求', { video_gen_id, model, content_count: content.length });

  const { withRateLimitRetry } = require('../utils/rateLimitRetry');
  const res = await withRateLimitRetry(async () => {
    const attemptRes = await fetch(base + '/v1/video_generation', {
      method: 'POST',
    ...
    const attemptRaw = await attemptRes.text();
    return { status: attemptRes.status, text: attemptRaw };
  }, { log, tag: `minimax:${model}`, maxRetries: 3, baseDelayMs: 5000 });
  const raw = res ? res.text : '';
  const respStatus = res ? res.status : 0;
  log.info('MiniMax视频原始响应', { video_gen_id, status: respStatus, raw: raw.slice(0, 1000) });
  if (!res || respStatus < 200 || respStatus >= 300) {
    let errMsg = 'MiniMax视频请求失败: ' + respStatus;
    try { const j = JSON.parse(raw); if (j.base_resp) errMsg += ' - ' + j.base_resp.status_msg; } catch (_) {}
    return { error: errMsg };
  }
  try {
    const j = JSON.parse(raw);
    const taskId = j.task_id || (j.base_resp && j.base_resp.task_id);
    if (taskId) return { task_id: taskId, status: 'PENDING' };
    if (j.file_id) return { video_url: base + '/v1/files/retrieve?file_id=' + j.file_id };
    return { error: 'MiniMax未返回task_id' };
  } catch (e) {
    return { error: 'MiniMax响应解析失败: ' + e.message };
  }
}
...
/**
 * ComfyUI视频生成API
 * 提交workflow到ComfyUI，返回prompt_id用于轮询
 */
async function callComfyUiVideoApi(config, log, opts) {
  const { prompt, negative_prompt, model, duration, aspect_ratio, image_url, first_frame_url, last_frame_url, 
          resolution, seed, storage_local_path, video_gen_id } = opts;

  const base = (config.base_url || '').replace(/\/$/, '');
  const promptUrl = base + (config.endpoint || '/prompt');

  // 读取workflow配置
  const settings = parseConfigSettingsJson(config);
  let workflow;

  // 优先从workflow_file加载workflow JSON文件
  if (settings.workflow_file || settings.workflow_path) {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        log.info('[ComfyUI] 从文件加载workflow', { file: filePath, video_gen_id });
      } else {
        log.warn('[ComfyUI] workflow文件不存在', { file: filePath, video_gen_id });
      }
    } catch (e) {
      log.warn('[ComfyUI] workflow文件加载失败', { file: filePath, error: e.message, video_gen_id });
    }
  }
  ...
  if (!workflow && settings.workflow_json) {
    ...
    try {
      ...
      log.info('[ComfyUI] 使用配置中的workflow_json', { video_gen_id });
    } catch (e) {
      log.warn('[ComfyUI] workflow_json解析失败', { error: e.message, video_gen_id });
    }
  }
  ...
  if (!workflow) {
    ...
    try {
      if (fs.existsSync(defaultWfPath)) {
        ...
        log.info('[ComfyUI] 使用默认workflow文件', { file: defaultWfPath, video_gen_id });
      }
    } catch (e) {
      log.warn('[ComfyUI] 默认workflow加载失败', { error: e.message, video_gen_id });
    }
  }
  ...
  try {
    ...
    if (!promptId) {
      log.error('[ComfyUI] 未返回prompt_id', { data: JSON.stringify(data).slice(0, 500), video_gen_id });
      return { error: 'ComfyUI未返回prompt_id: ' + JSON.stringify(data).slice(0, 300) };
    }

    log.info('[ComfyUI] Workflow已提交', { prompt_id: promptId, video_gen_id });
    ...
    if (node.inputs) {
      for (const [key, value] of Object.entries(node.inputs)) {
        if (typeof value === 'string') {
          ...
          if (newValue !== value) {
            // 对numeric字段转回数字，对boolean字段转回bool
            if (numericFields.has(key)) {
              const num = Number(newValue);
              node.inputs[key] = isNaN(num) ? newValue : num;
            } else if (key === 'cfg' || key === 'frame_rate') {
              const num = parseFloat(newValue);
              node.inputs[key] = isNaN(num) ? newValue : num;
            } else if (key === 'add_noise') {
              node.inputs[key] = newValue === 'true';
            } else {
              node.inputs[key] = newValue;
            }
          }
        }
      }
...
async function callVideoApi(db, log, opts) {
  ...
  // 成本追踪：成功返回前记录（best-effort，失败不影响主流程）
  const _recordVideoCost = () => {
    try { require('../middleware/costTracker').recordAuto(db, { service_type: 'video', provider: config.provider || 'openai', model: preferredModel, drama_id: opts.drama_id, metrics: { duration: opts.duration } }); } catch (_) {}
  };
  ...
  if (protocol === 'jimeng_ai_api') {
    const _r = await callJimengAiApiVideo(config, log, {
      prompt,
      model: preferredModel,
  ...
  if (protocol === 'xai') {
    const _r = await callXaiVideoApi(config, log, {
      prompt,
      model,
      duration: opts.duration,
      aspect_ratio,
  ...
  if (protocol === 'dashscope') {
    const _r = await callDashScopeVideoApi(config, log, {
      prompt,
      model,
      image_url: opts.image_url,
      first_frame_url: opts.first_frame_url,
  ...
  if (protocol === 'agnes') {
    const _r = await callAgnesVideoApi(config, log, {
      prompt, model: preferredModel, duration: opts.duration, aspect_ratio,
      resolution: opts.resolution, seed: opts.seed, image_url: opts.image_url,
      first_frame_url: opts.first_frame_url, last_frame_url: opts.last_frame_url,
      reference_urls: opts.reference_urls, files_base_url: opts.files_base_url,
  ...
  if (protocol === 'minimax') {
    const _r = await callMiniMaxVideoApi(config, log, {
      prompt, model: preferredModel, duration: opts.duration, aspect_ratio,
      resolution: opts.resolution, seed: opts.seed, image_url: opts.image_url,
      first_frame_url: opts.first_frame_url, last_frame_url: opts.last_frame_url,
      reference_urls: opts.reference_urls, files_base_url: opts.files_base_url,
  ...
  if (protocol === 'gemini') {
    const _r = await callGeminiVideoApi(config, log, {
      prompt, model,
      duration: opts.duration,
      aspect_ratio,
      image_url: opts.image_url,
  ...
  if (protocol === 'vidu') {
    const _r = await callViduVideoApi(config, log, {
      prompt, model,
      duration: opts.duration,
      aspect_ratio,
      resolution: opts.resolution,
  ...
  if (protocol === 'kling') {
    const _r = await callKlingVideoApi(config, log, {
      prompt, model,
      duration: opts.duration,
      aspect_ratio,
      image_url: opts.image_url,
  ...
  if (protocol === 'kling_omni') {
    const _r = await callKlingOmniVideoApi(applyKlingOmniEnvOverrides(config), log, {
      prompt,
      model,
      duration: opts.duration,
      aspect_ratio,
  ...
  if (protocol === 'volcengine_omni') {
    const _r = await callVolcengineOmniVideoApi(config, log, {
      prompt,
      model,
      duration: opts.duration,
      aspect_ratio,
  ...
  if (protocol === 'veo3') {
    const _r = await callVeo3VideoApi(config, log, {
      prompt, model,
      image_url: opts.image_url,
      storage_local_path: opts.storage_local_path,
      video_gen_id: opts.video_gen_id,
  ...
  if (protocol === 'sora') {
    const _r = await callSoraVideoApi(config, log, {
      prompt, model,
      duration: opts.duration,
      aspect_ratio,
      image_url: opts.image_url,
  ...
  // ComfyUI protocol (api_protocol = 'comfyui')
  if (protocol === 'comfyui') {
    const _r = await callComfyUiVideoApi(config, log, {
      prompt,
      model,
      duration: opts.duration,
      aspect_ratio,
  ...
  try {
    ...
    return { error: '视频接口响应解析失败: ' + e.message + ' | raw: ' + raw.slice(0, 200) };
  }
  ...
  return { error: '视频接口响应缺少 task_id 与 video_url。原始响应: ' + JSON.stringify(data).slice(0, 300) };
}

/**
 * 视频任务轮询：适配各上游的查询接口（Agnes / DashScope / 火山等）
 */
async function pollVideoTask(db, log, videoGenId, taskId, config, maxAttempts = 300, intervalMs = 3000) {
  const provider = (config.provider || '').toLowerCase();
  ...
  /** Agnes 轮询命中 429 的连续次数（用于退避日志与幅度控制） */
  let agnesPollBackoffStreak = 0;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (isKling) {
        ...
        // ComfyUI: 轮询 /history/{prompt_id}
        const comfyBase = (config.base_url || '').replace(/\/$/, '');
        url = `${comfyBase}/history/${encodeURIComponent(taskId)}`;
        headers = {};  // ComfyUI本地部署通常不需要认证
        log.info('[ComfyUI poll] 查询history', { url, video_gen_id: videoGenId });
      } else if (isMiniMax) {
        const miniBase = (config.base_url || '').replace(/\/$/, '');
        url = `${miniBase}/v1/video_generation/${encodeURIComponent(taskId)}`;
        headers = { Authorization: 'Bearer ' + (config.api_key || '') };
      } else {
        url = queryUrl();
        headers = { Authorization: 'Bearer ' + (config.api_key || '') };
      ...
      if (!res.ok) {
        ...
        // 官方错误码语义（agnes-video-25 错误处理）：
        //   404 = 视频 ID 不存在 —— 任务已失效，继续轮询毫无意义，立即短路失败。
        //   401/403 = Key 无效/过期 —— 重试也不会恢复，直接失败。
        //   429 = 请求频率超限 —— 官方建议「指数退避并降低轮询频率」，这里拉长间隔后再试。
        //   其他 5xx —— 服务端临时错误，继续轮询。
        if (res.status === 404) {
          return { error: '远端任务不存在(404)，视频 ID 已失效，需重新提交' };
        }
        if (res.status === 401 || res.status === 403) {
          return { error: 'Agnes 鉴权失败(' + res.status + ')，请检查 API Key：' + bodyLogged.slice(0, 200) };
        }
        if (res.status === 429) {
          // 官方文档：429 = 请求频率超限，建议「使用指数退避并降低轮询频率」。
          // 实测 Agnes 的查询接口与创建任务共用同一 RPM 预算（TokenPlan 视频档 5 RPM），
          // 因此轮询也会撞 429。这里退避幅度给足：先 ×2，第二次起 ×1.5，上限 60s。
          const next = intervalMs < 4000 ? intervalMs * 2 : Math.round(intervalMs * 1.5);
          intervalMs = Math.min(60000, Math.max(intervalMs + 1000, next));
          agnesPollBackoffStreak = Math.max(agnesPollBackoffStreak, 1) + 1;
          log.warn('[poll] 命中 429，降低轮询频率', {
            video_gen_id: videoGenId, new_interval_ms: intervalMs, streak: agnesPollBackoffStreak,
          });
      ...
      if (isComfyUi) {
        // ComfyUI /history/{prompt_id} 响应解析
        // 响应结构: { "prompt_id": { "outputs": { "node_id": { "videos": [...] } } } }
        const promptData = data[taskId];
        
        if (!promptData) {
          log.info('[ComfyUI poll] 任务尚未完成，继续轮询', { attempt, video_gen_id: videoGenId });
          continue;
        }
        ...
        if (promptData.error) {
          ...
          log.error('[ComfyUI poll] 任务失败', { error: errMsg, video_gen_id: videoGenId });
          return { error: 'ComfyUI任务失败: ' + errMsg.slice(0, 500) };
        }

        // 检查outputs是否存在（表示任务完成）
        const outputs = promptData.outputs;
        if (!outputs) {
          log.info('[ComfyUI poll] 任务执行中，尚未产生输出', { attempt, video_gen_id: videoGenId });
          continue;
        }
        ...
        if (videoFile) {
          ...
          if (videoFile.subfolder) params.append('subfolder', videoFile.subfolder);
          
          const videoUrl = `${base}/view?${params.toString()}`;
          log.info('[ComfyUI poll] 视频生成完成', { video_url: videoUrl, video_gen_id: videoGenId });
          return { video_url: videoUrl };
        }

        // 如果没有视频，检查是否有图片（某些workflow可能先输出图片）
        let imageFile = null;
        for (const [nodeId, nodeOutput] of Object.entries(outputs)) {
        ...
        if (imageFile) {
          log.info('[ComfyUI poll] 任务完成，但只找到图片输出（非视频）', { 
            image: imageFile.filename, 
            video_gen_id: videoGenId 
          });
          // 某些情况下可能真的是图片，但也可能是workflow配置问题
          // 继续轮询，也许视频会在后续产生
          continue;
        }

        log.warn('[ComfyUI poll] 任务完成但未找到视频文件', { 
          outputs: JSON.stringify(outputs).slice(0, 500),
          video_gen_id: videoGenId 
        });
        continue;
      }
      ...
      if (isMiniMax) {
        const mmStatus = (data?.status || data?.state || '').toLowerCase();
        log.info('[MiniMax poll] 状态', { video_gen_id: videoGenId, attempt, status: mmStatus, id: taskId });
        const mmVideoUrl = data?.video_url || data?.data?.video_url || data?.file_id;
        if (mmVideoUrl && isPlausibleHttpVideoUrl(mmVideoUrl)) {
          log.info('[MiniMax poll] 完成', { video_gen_id: videoGenId, video_url: mmVideoUrl });
          return { video_url: mmVideoUrl };
        }
        if (mmStatus === 'succeeded' || mmStatus === 'completed' || mmStatus === 'done' || mmStatus === 'success') {
          const finalUrl = mmVideoUrl || pickProxyVideoUrl(data);
          if (finalUrl && isPlausibleHttpVideoUrl(finalUrl)) {
            return { video_url: finalUrl };
          }
          return { error: 'MiniMax completed but no video_url: ' + JSON.stringify(data).slice(0, 300) };
        }
        if (mmStatus === 'failed' || mmStatus === 'error' || mmStatus === 'fail') {
          const msg = data?.error?.message || data?.message || data?.base_resp?.status_msg || 'MiniMax task failed';
          return { error: 'MiniMax: ' + String(msg).slice(0, 400) };
        }
        continue;
      }

      // ── Agnes 轮询：GET {origin}/agnesapi?video_id={id}&model_name={model} ──
      if (protocol === 'agnes') {
        const agnesStatus = (data.status || '').toLowerCase();
        const agnesVideoUrl = data.remixed_from_video_id || data.url || (data.metadata && data.metadata.url) || data.video_url;
        log.info('[Agnes poll] 状态', { video_gen_id: videoGenId, attempt, status: agnesStatus, has_url: !!agnesVideoUrl });
        if (agnesVideoUrl && isPlausibleHttpVideoUrl(agnesVideoUrl)) {
          log.info('[Agnes poll] 完成', { video_gen_id: videoGenId, video_url: agnesVideoUrl });
          return { video_url: agnesVideoUrl };
        }
        if (agnesStatus === 'completed' || agnesStatus === 'succeeded' || agnesStatus === 'done') {
          return { error: 'Agnes completed but no metadata.url: ' + JSON.stringify(data).slice(0, 300) };
        }
        if (agnesStatus === 'failed' || agnesStatus === 'error') {
          const msg = data.error || data.message || 'Agnes 任务失败';
          return { error: String(msg).slice(0, 500) };
        }
  ...
  return { error: '视频生成轮询超时，任务在限定时间内未完成' };
}
