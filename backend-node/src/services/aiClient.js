  ...
  return new Promise((resolve, reject) => {
    ...
    const req = mod.request(options, (res) => {
      ...
      res.on('end', () => {
        clearTimeout(timer);
        const raw = Buffer.concat(chunks).toString('utf-8');
        // headers 透传（用于 429 时读取 Retry-After）
        const headers = {};
        const rawHeaders = res.rawHeaders || [];
        for (let i = 0; i + 1 < rawHeaders.length; i += 2) {
          headers[String(rawHeaders[i]).toLowerCase()] = rawHeaders[i + 1];
        }
        resolve({ statusCode: res.statusCode || 0, raw, headers });
      });
      res.on('error', (e) => {
        clearTimeout(timer);
        reject(e);
...
/**
 * 文本生成限流：Agnes 文本 RPM 按官方 tokenplan（default 20 / enterprise 40 / TokenPlan 1000）。
 *
 * 与图片/视频链路保持一致，采用「主动排队 + 429 全局冷却 + 退避重试」三层防护：
 *   1) 请求前 acquireAgnesSlot({kind:'text'}) —— 按官方 RPM 主动控制发送节奏，避免撞限流；
 *   2) 命中 429 时 applyAgnesCooldown() —— 让整个进程一起退让，而非各自重试放大冲击；
 *   3) 仍失败则指数退避重试。
 *
 * 仅当上游是 Agnes 时启用主动排队/冷却；其他上游沿用纯 429 退避。
 */
async function withTextRateLimit(fn, log, tag = 'text', opts = {}) {
  const maxRetries = 4;
  const baseDelayMs = 3000;
  const { isAgnes = false } = opts;
    ...
    try {
      if (isAgnes) {
        try {
          ...
          await acquireAgnesSlot({ kind: 'text', log, tag });
        } catch (_) { /* 限流器异常不应阻断正常请求 */ }
      }
      return await fn();
    } catch (err) {
      const msg = (err.message || '').toLowerCase();
      const isRateLimit = msg.includes('429') || msg.includes('rate limit') || msg.includes('rate_limit') || msg.includes('限流') || msg.includes('too many requests');
      ...
      if (isAgnes) {
        try {
          ...
          agnes429Streak += 1;
          applyAgnesCooldown(agnes429Streak, { log, tag });
        } catch (_) {}
      }
      if (attempt >= maxRetries) throw err;

      const waitMs = baseDelayMs * Math.pow(2, attempt) + Math.floor(Math.random() * 1000);
      if (log && log.warn) {
        log.warn(`[rate-limit] ${tag} 收到限流(429)，第 ${attempt + 1}/${maxRetries} 次重试，等待 ${Math.round(waitMs / 1000)}s`, { error: err.message?.slice(0, 200) });
      }
...
async function generateText(db, log, serviceType, userPrompt, systemPrompt, options = {}) {
  const { model: preferredModel, temperature = 0.7, json_mode = false, min_max_tokens = null, streamCallback = null, scene_key = null, stream } = options;
  ...
  // 上游是否 Agnes —— 决定文本链路是否启用官方 RPM 主动排队与全局冷却
  // （与 imageClient / videoClient 的判定口径保持一致）
  const isAgnesText =
    String(config.api_protocol || '').toLowerCase() === 'agnes' ||
    /agnes/i.test(config.provider || '') ||
    /agnes/i.test(String(config.base_url || '')) ||
    /agnes/i.test(model || '');
  ...
  try {
    if (config.settings) {
      ...
      if (s && typeof s.stream === 'boolean') settingsStream = s.stream;
      if (s && typeof s.custom_headers === 'object') customHeaders = s.custom_headers;
      if (s && typeof s.custom_body_params === 'object') customBodyParams = s.custom_body_params;
    }
  } catch (_) {}

  // stream 优先级：调用方显式传入 > settings 配置 > 默认 true（流式速度快）
  const useStream = stream !== undefined ? stream : (settingsStream !== null ? settingsStream : true);
  ...
  body = { ...body, ...customBodyParams };
  const startMs = Date.now();
  const authHeaders = { Authorization: 'Bearer ' + (config.api_key || ''), ...customHeaders };
  ...
  if (useStream) {
    ...
    try {
      const resStream = await withTextRateLimit(() => postJSONStream(url, authHeaders, body, 60000, (receivedLen, event, accumulated) => {
      ...
      }), log, 'generateText-stream', { isAgnes: isAgnesText });
      content = resStream.body;
      if (!content || !String(content).trim()) {
        // 流式返回空内容（部分模型流式 SSE 不携带 content），自动降级非流式重试
        log.warn('AI stream returned empty content, auto-fallback to non-stream', { model, url: url.slice(0, 60) });
        try {
          const resNonStream = await withTextRateLimit(() => postJSONNonStream(url, authHeaders, body, 180000), log, 'generateText-empty-fallback', { isAgnes: isAgnesText });
          content = resNonStream.body ?? null;
        } catch (fallbackErr) {
          log.error('AI non-stream fallback failed after empty stream', { error: fallbackErr.message, model });
        }
      }
    } catch (streamErr) {
      // 自动降级：如果流式调用失败且错误与 "stream" 相关，自动重试非流式
      const errMsg = (streamErr.message || '').toLowerCase();
      if (errMsg.includes('stream') || errMsg.includes('sse') || errMsg.includes('400') || errMsg.includes('403') || errMsg.includes('405')) {
        log.warn('AI stream failed, auto-fallback to non-stream', { model, original_error: streamErr.message?.slice(0, 200) });
        try {
          const resNonStream = await withTextRateLimit(() => postJSONNonStream(url, authHeaders, body, 180000), log, 'generateText-fallback', { isAgnes: isAgnesText });
          if (resNonStream.body != null) {
            content = resNonStream.body;
          } else {
            // postJSONNonStream 可能因 JSON 解析失败返回 { body: null, raw }
            log.warn('AI non-stream fallback raw response', { raw_preview: (resNonStream.raw || '').slice(0, 200) });
            throw new Error('AI 非流式返回内容为空: ' + (resNonStream.raw || '').slice(0, 200));
          }
        } catch (fallbackErr) {
          log.error('AI non-stream fallback also failed', { error: fallbackErr.message });
          throw streamErr; // 抛出原始错误，保留语义
        }
      } else {
        throw streamErr; // 非 stream 相关的错误，正常抛出
      }
    }
  } else {
    // 路径 B：非流式调用（兼容不支持 stream 的 API）
    log.info('AI generateText request (non-stream)', { url: url.slice(0, 60), model, max_tokens: finalMaxTokens ?? '(model default)', json_mode, stream: false });
    const resNonStream = await withTextRateLimit(() => postJSONNonStream(url, authHeaders, body, 180000), log, 'generateText-nonstream', { isAgnes: isAgnesText });
    if (resNonStream.body != null) {
      content = resNonStream.body;
    } else {
      log.warn('AI non-stream raw response', { raw_preview: (resNonStream.raw || '').slice(0, 200) });
      throw new Error('AI 返回内容为空: ' + (resNonStream.raw || '').slice(0, 200));
    }
  }
  ...
  try { require('../middleware/costTracker').recordAuto(db, { service_type: 'text', provider: config.provider || 'openai', model, metrics: { tokens: content.length } }); } catch (_) {}
  return content;
}
...
async function streamGenerateText(db, log, serviceType, userPrompt, systemPrompt, options = {}, onDelta) {
  ...
  // 上游是否 Agnes —— 文本链路启用官方 RPM 主动排队与全局冷却（与 generateText 口径一致）
  const isAgnesText =
    String(config.api_protocol || '').toLowerCase() === 'agnes' ||
    /agnes/i.test(config.provider || '') ||
    /agnes/i.test(String(config.base_url || '')) ||
  ...
  body = applyDeepSeekChatOptions(config, body);
  const authHeaders = { Authorization: 'Bearer ' + (config.api_key || '') };
  ...
  // 尝试流式，失败自动降级为非流式
  try {
    log.info('AI streamGenerateText request', { url: url.slice(0, 60), model, max_tokens: finalMaxTokens ?? '(model default)', json_mode, stream: true });
    let lastLen = 0;
    const res = await withTextRateLimit(() => postJSONStream(url, authHeaders, body, silenceMs, (receivedLen, event, accumulated) => {
      if (event === 'first_token') {
        log.info('AI stream first token', { model, ttft_ms: Date.now() - startMs });
      }
    ...
    }), log, 'streamGenerateText-stream');
    content = res.body;
    if (!content || !String(content).trim()) {
      log.warn('AI streamGenerateText: stream returned empty content, auto-fallback to non-stream', { model, url: url.slice(0, 60) });
      const resNonStream = await withTextRateLimit(() => postJSONNonStream(url, authHeaders, body, 180000), log, 'streamGenerateText-empty-fallback', { isAgnes: isAgnesText });
      content = resNonStream.body ?? null;
    }
  } catch (streamErr) {
    const errMsg = (streamErr.message || '').toLowerCase();
    if (errMsg.includes('stream') || errMsg.includes('sse') || errMsg.includes('400') || errMsg.includes('403') || errMsg.includes('405')) {
      log.warn('AI streamGenerateText: stream failed, auto-fallback to non-stream', { model, original_error: streamErr.message?.slice(0, 200) });
      const resNonStream = await withTextRateLimit(() => postJSONNonStream(url, authHeaders, body, 180000), log, 'streamGenerateText-fallback', { isAgnes: isAgnesText });
      if (resNonStream.body != null) {
        content = resNonStream.body;
      } else {
        throw new Error('AI 非流式返回内容为空: ' + (resNonStream.raw || '').slice(0, 200));
