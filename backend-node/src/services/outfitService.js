  ...
  const service = {
    ...
    create(characterId, dramaId, data = {}) {
      const {
        ...
        accessory, signature_accessory = accessory,
      } = data;
      ...
      `).run(
        ...
        hair_style || null, face_shape || null, body_type || null, skin_tone || null,
        signature_accessory || null, sortOrder, isDefault, now, now
      );
    ...
    remove(outfitId) {
      ...
      if ((existing.is_default === true || existing.is_default === 1) && totalForChar.cnt > 1) {
