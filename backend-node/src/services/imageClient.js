...
/**
 * Agnes 图片 size 归一化（官方文档 agnes-image-21-flash）。
 *
 * 官方规定 size 为输出尺寸**档位**：`1K` / `2K` / `3K` / `4K`，并配合 `ratio` 使用。
 * 传 "720P"、1792x1024 等会 400：`size has invalid value "720P"; must be one of: 1K, 2K, 3K, 4K, or WIDTHxHEIGHT`。
 * 官方输出对照表（宽 x 高）：16:9 → 1K 1312x736 / 2K 2624x1472；9:16 → 1K 736x1312 / 2K 1472x2624 等。
 *
 * 策略：把任意输入映射到「最接近的官方档位 + ratio」，避免被服务端标准化成意外尺寸。
 * @param {string} size - 原始尺寸（"1K" / "720P" / "1792x1024" / "2560x1440" ...）
 * @returns {{ size: string, ratio: string|null, changed: boolean }}
 */
const AGNES_SIZE_TIERS = ['1K', '2K', '3K', '4K'];
// 官方支持且按「最长边」排序的 ratio（用于把像素尺寸反推成最接近的官方比例）
const AGNES_RATIOS = [
...
function normalizeAgnesImageSize(size) {
  const raw = String(size || '').trim();
  if (!raw) return { size: '1K', ratio: null, changed: true };

  // 1) 已是合法档位
  const upper = raw.toUpperCase();
  if (AGNES_SIZE_TIERS.includes(upper)) {
    return { size: upper, ratio: null, changed: upper !== raw };
  }

  // 2) 视频档位 / 常见显示器名 → 映射到图片档位
  if (/^\d{3,4}P$/i.test(raw)) {
    const p = parseInt(raw, 10);
    // 720P/1080P 归 1K，1440P 归 2K，2160P 归 4K
    const tier = p >= 2000 ? '4K' : p >= 1400 ? '2K' : '1K';
    return { size: tier, ratio: null, changed: true };
  }

  // 3) 精确像素 WxH → 档位 + 最接近的官方 ratio
  const m = raw.toLowerCase().replace(/\s/g, '').match(/^(\d+)[x*](\d+)$/);
  if (m) {
    const w = parseInt(m[1], 10);
    const h = parseInt(m[2], 10);
    if (w > 0 && h > 0) {
      const longEdge = Math.max(w, h);
      // 官方各档最长边：1:1 为 1024/2048/3072/4096，16:9 为 1312/2624/3936/5248。
      // 以本档最大值为界，保证官方尺寸精确落档（4096x4096 必须是 4K）。
      const tier = longEdge <= 1312 ? '1K' : longEdge <= 2624 ? '2K' : longEdge <= 3936 ? '3K' : '4K';
      const want = w / h;
      let best = AGNES_RATIOS[0];
      let bestDiff = Infinity;
      for (const r of AGNES_RATIOS) {
        const diff = Math.abs(r.value - want);
        if (diff < bestDiff) { bestDiff = diff; best = r; }
      }
      return { size: tier, ratio: best.ratio, changed: true };
    }
  }

  // 4) 无法识别 → 官方默认 1K（ratio 交回服务端默认 1:1）
  return { size: '1K', ratio: null, changed: true };
}
...
/**
 * Agnes 图片参考图解析（官方文档明确支持 Data URI Base64 输入）。
 *
 * 关键：Agnes 是云端服务，无法访问 http://localhost:5679/static/... 。
 * 历史实现把本地图经第三方图床（imageproxy.zhongzhuan.chat）转公网 URL，
 * 图床一不稳定就成批失败，报错形如：
 *   `500 - upload image queue input image[1]: download input image failed: request reject: port 5679 is not allowed`
 *   `400 - upload image queue input image[0]: image queue input invalid: download input image failed: HTTP 404`
 *
 * 官方文档「输入图像 URL 无法访问」一节的建议就是：如果无法公开访问，请使用 Data URI Base64。
 * 因此这里统一优先转 base64（无外部依赖），只有公网 URL 才原样透传（避免无谓的体积膨胀）。
 *
 * @returns {string|null} data URI 或公网 URL
 */
function resolveAgnesImageRef(value, filesBaseUrl, storageLocalPath, log) {
  if (!value || !String(value).trim()) return null;
  const s = String(value).trim();
  if (s.startsWith('data:')) return s;
  const baseUrl = (filesBaseUrl || '').replace(/\/$/, '');

  // 已是公网 URL（且不是本机）→ 官方文档说明必须「不需要登录/cookie/私有请求头」，
  // 这类直传由上游校验；保持原样，不再多绕一层图床。
  const isLocalhostUrl = /localhost|127\.0\.0\.1|0\.0\.0\.0/i.test(s);
  if (/^https?:\/\//i.test(s) && !isLocalhostUrl) {
    return s;
  }

  // 本地 / localhost → 读文件转 Data URI Base64
  let relPath = null;
  if (/^https?:\/\//i.test(s)) {
    const afterStatic = s.split('/static/')[1]
      || (baseUrl ? s.replace(baseUrl + '/', '').replace(baseUrl, '') : null)
  ...
  try {
    if (!fs.existsSync(filePath)) {
      if (log && log.warn) log.warn('[Agnes图生] 参考图文件不存在，已跳过', { src: s.slice(0, 120), filePath });
      return null;
    }
    const buf = fs.readFileSync(filePath);
    const ext = path.extname(filePath).toLowerCase();
    ...
    if (log && log.warn) log.warn('[Agnes图生] 参考图读取失败，已跳过', { src: s.slice(0, 120), error: e.message });
    return null;
  }
}
...
async function callImageApi(db, log, opts) {
  ...
  const _recordImageCost = () => {
    try { require('../middleware/costTracker').recordAuto(db, { service_type: 'image', provider: config.provider || 'openai', model: preferredModel, drama_id: opts.drama_id, metrics: {} }); } catch (_) {}
  };
  ...
  if (protocol === 'comfyui') {
    const _r = await callComfyUIImageApi(config, log, {
  ...
  if (protocol === 'dashscope') {
    const _r = await callDashScopeImageApi(config, log, {
      prompt: effectivePrompt, model, size, image_gen_id,
      reference_image_urls: opts.reference_image_urls,
      files_base_url: opts.files_base_url,
      storage_local_path: opts.storage_local_path,
  ...
  if (protocol === 'nano_banana') {
    const _r = await callNanoBananaImageApi(config, log, {
      prompt: effectivePrompt, model, size, image_gen_id,
      reference_image_urls: opts.reference_image_urls,
      files_base_url: opts.files_base_url,
      storage_local_path: opts.storage_local_path,
  ...
  if (protocol === 'kling') {
    const _r = await callKlingImageApi(config, log, {
      prompt: effectivePrompt, model, size, image_gen_id,
      reference_image_urls: opts.reference_image_urls,
      files_base_url: opts.files_base_url,
      storage_local_path: opts.storage_local_path,
  ...
  if (protocol === 'gemini') {
    const _r = await callGeminiImageApi(db, config, log, {
  ...
  const isAgnesImageProtoEarly = protocol === 'agnes' || /agnes/i.test(provider || '') || /agnes-image/i.test(model || '');
  // 解析参考图：本地路径/localhost URL → base64，公网 URL → 直接传
  // Agnes 优先走专用解析器（官方支持 Data URI Base64，绕开第三方图床，规避 localhost 无法访问导致的成批失败）
  const rawRefs = Array.isArray(reference_image_urls) ? reference_image_urls.filter(Boolean) : [];
  const resolvedRefs = rawRefs
    .map((r) => (isAgnesImageProtoEarly
      ? resolveAgnesImageRef(r, files_base_url, storage_local_path, log)
      : resolveImageRef(r, files_base_url, storage_local_path)))
    .filter(Boolean);
  if (rawRefs.length > 0 && resolvedRefs.length < rawRefs.length) {
    log.warn('[图生] 部分参考图无法解析，已丢弃', {
      image_gen_id, total: rawRefs.length, resolved: resolvedRefs.length,
    });
  }
  if (resolvedRefs.length > 0) {
  ...
  let effectiveSize = (isSeedream && size) ? fixSeedreamSize(size) : size;
  // Agnes 图片 API（官方文档 https://www.agnes-ai.cn/zh-Hans/docs/agnes-image-21-flash）：
  //  size 必须是输出档位 1K / 2K / 3K / 4K，配合 ratio 使用；传 "720P" 或任意像素尺寸会 400
  //  （size has invalid value "720P"; must be one of: 1K, 2K, 3K, 4K, or WIDTHxHEIGHT）。
  //  历史精确尺寸（如 1792x1024、2560x1440）官方虽兼容但可能被标准化，统一转成档位更稳。
  const isAgnesImageProto = protocol === 'agnes' || /agnes/i.test(provider || '') || /agnes-image/i.test(model || '');
  let agnesRatio = null;
  if (isAgnesImageProto) {
    const norm = normalizeAgnesImageSize(effectiveSize);
    effectiveSize = norm.size;
    agnesRatio = norm.ratio;
    if (norm.changed) {
      log.info('[图生] Agnes size 已按官方文档归一化为档位', {
        image_gen_id, original_size: size, size: effectiveSize, ratio: agnesRatio,
      });
    }
  }
  ...
  const body = {
    ...
    // Agnes 图片：档位式 size 需配合 ratio（官方示例 { size: "2K", ratio: "16:9" }）
    ...(isAgnesImageProto && agnesRatio ? { ratio: agnesRatio } : {}),
    ...
    // Agnes 图片 API 要求 image 放在 extra_body 中（顶层会被忽略，执行文生图）
    ...((() => {
      if (resolvedRefs.length === 0) return {};
      if (isAgnesImageProtoEarly) return { extra_body: { image: resolvedRefs, response_format: 'url' } };
      return { image: resolvedRefs };
    })()),
  };
  ...
  let httpStatus;
  try {
    // Agnes 严格按官方 RPM 主动限速（tokenplan 文档），其余上游沿用 429 退避重试。
    const { withRateLimitRetry } = require('../utils/rateLimitRetry');
    const isAgnesUpstream = protocol === 'agnes' || /agnes/i.test(provider || '') || /agnes/i.test(model || '');
    let agnes429Streak = 0;
    const out = await withRateLimitRetry(async () => {
      if (isAgnesUpstream) {
        // 官方：图片 RPM 按输出分辨率档位区分（1K/2K/3K/4K），主动排队避免撞限流
        const { acquireAgnesSlot } = require('../utils/agnesRateLimiter');
        await acquireAgnesSlot({
          kind: 'image',
      ...
      const attemptRes = await postJSONWithTimeout(url, openaiCompatHeaders, body, IMAGE_HTTP_TIMEOUT_MS);
      if (isAgnesUpstream && attemptRes.statusCode === 429) {
        // 官方建议：429 时指数退避并降低频率 —— 用全局冷却让整个进程一起退让
        const { applyAgnesCooldown } = require('../utils/agnesRateLimiter');
        agnes429Streak += 1;
        applyAgnesCooldown(agnes429Streak, { log, tag: `image:${model}` });
      }
      return { status: attemptRes.statusCode, raw: attemptRes.raw, headers: attemptRes.headers || null, detail: attemptRes };
    }, {
      log,
      tag: `image:${model}`,
      // Agnes 已有全局冷却+主动排队，重试次数给足；其他上游保持原样
      maxRetries: isAgnesUpstream ? 5 : 3,
      baseDelayMs: isAgnesUpstream ? 10000 : 5000,
      maxDelayMs: 120000,
    });
...
async function callComfyUIImageApi(config, log, opts) {
  ...
  // 这对远程 GPU 服务器场景至关重要——远程服务器无法访问本地 localhost URL。
  if (isI2I && imageUrl) {
    try {
