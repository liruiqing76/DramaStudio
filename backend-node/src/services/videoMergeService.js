...
/** 使用 ffmpeg concat 合并多个视频文件；流拷贝失败时自动降级为重新编码，保证合成完整 */
function runFfmpegConcat(localPaths, outputPath, log) {
  ...
  try {
    ...
    // 1) 流拷贝（快、无损；要求各段编码/分辨率/timebase 一致）
    const copyArgs = ['-f', 'concat', '-safe', '0', '-i', listFile, '-c', 'copy', '-y', outputPath];
    let result = spawnSync(ffmpegBin, copyArgs, spawnOpts);
    if (!result.error && result.status === 0 && fs.existsSync(outputPath)) return true;
    log.warn('Video merge: stream copy failed, trying re-encode', {
      error: result.error && result.error.message, stderr: result.stderr && result.stderr.slice(-500),
    });

    // 2) 重新编码统一格式（慢，但兼容编码/分辨率轻微不一致；失败则由调用方降级）
    ...
    result = spawnSync(ffmpegBin, reencodeArgs, spawnOpts);
    if (!result.error && result.status === 0 && fs.existsSync(outputPath)) {
      log.info('Video merge: re-encode concat succeeded');
      return true;
    }
    log.warn('Video merge: re-encode concat also failed', {
      error: result.error && result.error.message, stderr: result.stderr && result.stderr.slice(-500),
    });
    return false;
  } finally {
...
async function processVideoMerge(db, log, mergeId, baseUrl) {
  ...
  // BGM 混入：从 episode 读取 bgm_path（仅当 episodes 表有 bgm 列时生效）
  if (mergedRelativePath && ffmpegAvailable) {
    try {
      const epCols = db.prepare("PRAGMA table_info(episodes)").all();
      const bgmCol = epCols.find(c => c.name === 'bgm_path' || c.name === 'bgm_url' || c.name === 'bgm');
      if (bgmCol) {
        const epRow = db.prepare(`SELECT ${bgmCol.name} AS bgm_val FROM episodes WHERE id = ?`).get(episodeId);
        const bgmVal = epRow?.bgm_val;
        if (bgmVal && String(bgmVal).trim()) {
          const mergedAbsPath = path.join(storageRoot, mergedRelativePath.replace(/\//g, path.sep));
          let bgmAbsPath = bgmVal;
          if (!path.isAbsolute(bgmVal) || !fs.existsSync(bgmVal)) {
            bgmAbsPath = path.join(storageRoot, String(bgmVal).replace(/^\//, '').replace(/\//g, path.sep));
          }
          if (fs.existsSync(bgmAbsPath)) {
            const bgmOutputPath = mergedAbsPath.replace(/\.mp4$/i, '_bgm.mp4');
            const mediaFfmpeg = require('../adapters/media/ffmpeg');
            const bgmResult = mediaFfmpeg.addBgm(mergedAbsPath, bgmAbsPath, bgmOutputPath, { log });
            if (bgmResult.ok && fs.existsSync(bgmOutputPath)) {
              try { fs.unlinkSync(mergedAbsPath); } catch (_) {}
              fs.renameSync(bgmOutputPath, mergedAbsPath);
              log.info('Video merge: BGM mixed in', { merge_id: mergeId, bgm: bgmVal });
            } else {
              log.warn('Video merge: BGM mix failed, keeping original', { merge_id: mergeId, error: bgmResult.error });
            }
          } else {
            log.warn('Video merge: BGM file not found', { merge_id: mergeId, bgm: bgmVal });
          }
        }
      }
    } catch (e) {
      log.warn('Video merge: BGM step skipped', { merge_id: mergeId, error: e.message });
    }
  }
  ...
  if (!mergedRelativePath) {
    log.warn('Video merge FAILED: concat 不可用，合成结果仅保留首段分镜，请检查 ffmpeg 或各分镜编码/分辨率是否一致', { merge_id: mergeId, episode_id: episodeId, clip_count: scenes.length });
  }
}
