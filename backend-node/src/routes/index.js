...
const templates = templateRoutes.routes(db, cfg, log);
  ...
  // ---------- semantic-search ----------
  r.get('/semantic-search', (req, res) => {
    const { query, type, drama_id } = req.query;
    if (!query || !type) return response.badRequest(res, 'query 和 type 必填');
    if (!['character', 'scene', 'prop'].includes(type)) return response.badRequest(res, 'type 需为 character/scene/prop');

    const vm = require('../services/vectorMemory');
    let rows = [];

    if (type === 'character') {
      const sql = drama_id
        ? 'SELECT id, name, description, image_url FROM characters WHERE deleted_at IS NULL AND drama_id = ?'
        : 'SELECT id, name, description, image_url FROM character_library WHERE deleted_at IS NULL';
      rows = db.prepare(sql).all(...(drama_id ? [Number(drama_id)] : []));
    } else if (type === 'scene') {
      const sql = drama_id
        ? 'SELECT id, name, description, image_url FROM scenes WHERE deleted_at IS NULL AND drama_id = ?'
        : 'SELECT id, name, description, image_url FROM scene_library WHERE deleted_at IS NULL';
      rows = db.prepare(sql).all(...(drama_id ? [Number(drama_id)] : []));
    } else {
      const sql = drama_id
        ? 'SELECT id, name, description, image_url FROM props WHERE deleted_at IS NULL AND drama_id = ?'
        : 'SELECT id, name, description, image_url FROM prop_library WHERE deleted_at IS NULL';
      rows = db.prepare(sql).all(...(drama_id ? [Number(drama_id)] : []));
    }
    ...
    vm.rebuildIndex(items);
    const results = vm.search(query, items.map(i => i.id), 50);
    res.json({
      success: true,
      results: results.map(r => ({
        ...items.find(i => i.id === r.id).raw,
