  ...
  return {
    ...
    create: (req, res) => {
      try {
        ...
        // provider 仅用于记录/展示。前端（FilmCreate.vue / FreeCreate.vue）的 5 处
        // videosAPI.create 调用都不传 provider，历史上这里硬编码 'chatfire'，
        // 导致库里所有 video_generations.provider 都记成了 chatfire（与实际使用的
        // Agnes 不符，排查/统计失真）。改为按当前默认视频配置推断真实 provider。
        let provider = body.provider;
        if (!provider) {
          try {
            const cfgVideo = videoClient.getDefaultVideoConfig(db);
            provider = (cfgVideo && (cfgVideo.provider || cfgVideo.ai_provider)) || 'openai';
          } catch (_) {
            provider = 'openai';
          }
        }
