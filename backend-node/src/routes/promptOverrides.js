...
const PROMPT_META = [
  {
    ...
    label: '故事生成提示词',
    description: '控制 AI 如何将故事梗概扩写成完整剧本（含六拍结构骨架与人物一致性锚点；题材模板会自动追加）',
  },
  {
    key: 'storyboard_system',
    label: '分镜拆解提示词',
    description: '控制 AI 如何将剧本拆分成分镜头方案（含可拍性铁律与首尾帧校验；输出格式要求已锁定）',
  },
  {
    key: 'character_extraction',
    label: '角色提取提示词',
    description: '控制 AI 如何从剧本中提取角色信息，输出 6 维身份锚点（供生图跨镜头一致性使用；输出格式已锁定）',
  },
  {
    key: 'scene_extraction',
    label: '场景提取提示词',
    description: '控制 AI 如何从剧本中提取场景背景，输出场景固定锚点（供生图跨镜头一致性使用；风格/比例和输出格式已锁定）',
  },
  {
    key: 'prop_extraction',
    label: '道具提取提示词',
  ...
  },
  {
    ...
    label: '分镜输出格式要求',
    description: '追加在分镜拆解用户提示词末尾的详细要素说明（含可拍性与首尾帧要求；JSON 输出格式已锁定）',
  },
  {
...
// default_body 和 locked_suffix 优先从 skills/*.md 读取（单一默认源）；文件缺失时回退到 PROMPT_META + promptI18n 硬编码
function getPromptDefinitions() {
  const fromSkills = skillService.listSkills();
