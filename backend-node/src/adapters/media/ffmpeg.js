...
function concatVideos(inputs, output, opts = {}) {
  ...
  const args = ['-y', ...inputs.flatMap((p) => ['-i', escapeFfmpegPath(p)]),
...
function addBgm(video, bgmPath, output, opts = {}) {
  ...
  const args = [
    ...
    `[1:a]volume=${bgmVolume},afade=t=in:st=0:d=${fadeDur},afade=t=out:st=${fadeDur}[bgm];` +
...
function addCrossFade(inputs, output, opts = {}) {
  ...
  // 构建 filter_complex：N 段视频 + N-1 个交叉淡入淡出
  const filters = inputs.map((p, i) => {
    const arg = `-i ${escapeFfmpegPath(p)}`;
    return { arg, idx: i };
  });
  // xfade 链：每两段之间加 xfade，overlap = duration
  let filterChain = '';
  for (let i = 0; i < inputs.length - 1; i++) {
    filterChain += `[${i}:v][${i + 1}:v]xfade=transition=fade:duration=${duration}:offset=${duration * i}[v${i}]`;
    if (i < inputs.length - 2) filterChain += ';';
  }
  const mapOut = inputs.length === 2 ? '[v0]' : `[v${inputs.length - 2}]`;
  const args = [
    '-y',
...
function colorGrade(input, output, opts = {}) {
  ...
  const filterComplex = `[0:v]${filters.join('=')}${filters.join(',')}[v]`;
  const args = [
