...
function getBackendCwd() {
  if (app.isPackaged) {
    // 优先从 exe 向上查找项目源码目录（backend-node），找到则数据归项目目录
    let dir = path.dirname(process.execPath);
    for (let i = 0; i < 6; i++) {
      const candidate = path.join(dir, 'backend-node');
      if (fs.existsSync(path.join(candidate, 'src', 'app.js'))) {
        return candidate;
      }
    ...
    // 回退：portable 模式，数据放 exe 同级目录
    return path.join(path.dirname(process.execPath), 'backend');
  }
