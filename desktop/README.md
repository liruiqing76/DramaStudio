...
| 分镜全能模式 | 分镜可在**经典**与**全能模式**间切换；全能模式中间为**片段描述**（`@图片1`… 多图参考），配合参考图生视频链路（如 Agnes 参考图模式）；生视频前校验配置匹配；支持「根据分镜生成提示词」 |
...
| 文件 | 说明 |
|------|------|
| `DramaStudio Setup x.x.x.exe` | NSIS 安装包（有安装引导，可选安装目录） |
| `DramaStudio x.x.x.exe` | 便携版（单文件，无需安装，双击即用） |

首次运行时，会在用户数据目录（如 `%APPDATA%/dramastudio-desktop`）下生成 `backend/`，包含 `configs/config.yaml`（从 example 复制）和 `data/`（数据库与文件存储），按需修改配置即可。
...
```
%APPDATA%\DramaStudio\backend\logs\app.log
```
...
```powershell
& "D:\path\to\release\DramaStudio 1.2.7.exe"
```
...
```
%APPDATA%\DramaStudio\backend\configs\config.yaml
```
