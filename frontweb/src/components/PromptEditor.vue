            ...
            >
              <div class="menu-item-content">
                ...
                <el-tag
                  ...
                  size="small"
                  class="menu-tag"
                >{{ $t('promptEditor.customized') }}</el-tag>
                <el-tag v-else type="info" size="small" class="menu-tag">{{ $t('promptEditor.default') }}</el-tag>
              </div>
          ...
          <div v-if="currentPrompt" class="prompt-card">
            <div class="prompt-card-header">
              <div class="prompt-card-meta">
                ...
                <el-tag
                  ...
                  class="custom-tag"
                >{{ $t('promptEditor.customized') }}</el-tag>
                <el-tag v-else type="info" size="small" class="custom-tag">{{ $t('promptEditor.useDefault') }}</el-tag>
              </div>
            ...
            <div class="prompt-edit-section">
              <div class="section-label">
                ...
                <span>{{ $t('promptEditor.editableContent') }}</span>
              </div>
              <el-input
            ...
            <div v-if="currentPrompt.locked_suffix" class="prompt-locked-section">
              <div class="section-label section-label--locked">
                ...
                <span>{{ $t('promptEditor.lockedContent') }}</span>
              </div>
...
async function load() {
  ...
  try {
    ...
    ElMessage.error(t('promptEditor.loadFailed'))
  } finally {
    loading.value = false
  }
}
...
async function save(p) {
  ...
  if (!content?.trim()) {
    ElMessage.warning(t('promptEditor.contentEmpty'))
    return
  }
  ...
  try {
    ...
    ElMessage.success(t('promptEditor.saved'))
  } catch (_) {
  } finally {
...
async function reset(p) {
  await ElMessageBox.confirm(t('promptEditor.confirmReset', { label: p.label }), t('promptEditor.resetDefault'), {
    type: 'warning',
  })
  ...
  try {
    ...
    isDirty.value[p.key] = false
    ElMessage.success(t('promptEditor.resetDone'))
  } catch (_) {
  } finally {
