<template>
  <div class="style-picker-wrap">
    ...
    >
      ...
      <span v-else class="spt-placeholder">{{ placeholder || $t('stylePicker.placeholderDefault') }}</span>
    ...
    <el-dialog
      v-model="visible"
      :title="$t('stylePicker.dialogTitle')"
      width="90vw"
    ...
    >
      <div class="spd-search">
        ...
        <span v-if="modelValue" class="spd-selected-hint">
          {{ $t('stylePicker.selectedHint', { label: selectedOption?.label }) }}
        </span>
      </div>
      ...
      <div class="spd-body">
        ...
        <div v-if="filteredGroups.length === 0" class="spd-empty">{{ $t('stylePicker.noMatch') }}</div>
      </div>

      <template #footer>
        <el-button @click="clearAndClose">{{ $t('stylePicker.clearSelection') }}</el-button>
        <el-button type="primary" @click="visible = false">{{ $t('stylePicker.done') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
