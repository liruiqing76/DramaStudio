<template>
  <div class="omni-at-wrap" ref="wrapRef">
    <div
      ...
      spellcheck="false"
      :data-placeholder="$t('segmentEditor.placeholder')"
      @input="onInput"
      @blur="onBlur"
      @keydown="onKeydown"
      @paste="onPaste"
    ...
    <teleport to="body">
      <div
        ...
        @mousedown.prevent
      >
        <div v-if="!slots.length" class="omni-at-menu-empty">{{ $t('segmentEditor.noSlots') }}</div>
        <button
          v-for="s in slots"
          :key="s.index"
          type="button"
        ...
        >
          ...
          <span class="omni-at-menu-meta">
            ...
            <span class="omni-at-menu-at-sub">{{ $t('segmentEditor.submitPrefix') }} {{ canonicalAt(s.index) }}</span>
          </span>
        </button>
      </div>
    </teleport>
    <div class="omni-at-footer">
      <el-tooltip :content="$t('segmentEditor.copyTooltip')" placement="top">
        <el-button type="default" text size="small" class="omni-at-copy-btn" @click="onCopyCanonical">
          <el-icon><DocumentCopy /></el-icon>
          {{ $t('segmentEditor.copyPrompt') }}
        </el-button>
      </el-tooltip>
    </div>
  </div>
...
function kindLabel(kind) {
  if (kind === 'scene') return t('segmentEditor.kindScene')
  if (kind === 'character') return t('segmentEditor.kindCharacter')
  if (kind === 'prop') return t('segmentEditor.kindProp')
  return t('segmentEditor.kindRef')
}
...
function makeDisplayAtToken(index) {
  ...
  if (!dup) return `@${name}`
  const prefix = slot.kind === 'scene' ? t('segmentEditor.kindScene') : slot.kind === 'prop' ? t('segmentEditor.kindProp') : t('segmentEditor.kindCharacter')
  return `@${prefix}·${name}`
}
...
function applyPlainTextToEditor(el, text) {
  ...
  while ((m = re.exec(raw)) !== null) {
    ...
    span.setAttribute('aria-label', t('segmentEditor.ariaLabel', { disp, canonical: canonicalAt(m[1]) }))
...
function onPickSlot(index) {
  ...
  if (menuMode === 'replace' && replaceChipEl) {
    ...
    replaceChipEl.setAttribute('aria-label', t('segmentEditor.ariaLabel', { disp, canonical: canonicalAt(index) }))
    const next = serializeEditor(el)
    skipNextModelWatch = true
    emit('update:modelValue', next)
    closeMenu()
...
watch(
  ...
  () => {
    ...
    el.querySelectorAll(`.${CHIP_CLASS}`).forEach((chip) => {
      ...
      chip.setAttribute('aria-label', t('segmentEditor.ariaLabel', { disp, canonical: canonicalAt(n) }))
    })
  },
  { deep: true }
)
...
async function onCopyCanonical() {
  ...
  try {
    ...
    ElMessage.success(t('segmentEditor.copied'))
  } catch (_) {
    try {
      ...
      ElMessage.success(t('segmentEditor.copiedShort'))
    } catch (e2) {
      ElMessage.error(e2?.message || t('segmentEditor.copyFailed'))
    }
  }
}
