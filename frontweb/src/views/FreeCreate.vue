  ...
  <div class="free-create-page">
    <div class="page-header">
      <div class="header-left">
        ...
        <h2 class="page-title">{{ $t('freeCreate.pageTitle') }}</h2>
      </div>
      <p class="page-desc">{{ $t('freeCreate.pageDesc') }}</p>
    </div>
    ...
    <div class="create-layout">
      ...
      <div class="input-panel">
        <el-tabs v-model="mode" class="mode-tabs">
          <el-tab-pane :label="$t('freeCreate.tabImage')" name="image" />
          <el-tab-pane :label="$t('freeCreate.tabVideo')" name="video" />
        </el-tabs>

        <div class="form-section">
          <div class="form-label">{{ $t('freeCreate.promptLabel') }} <span class="required">*</span></div>
          <el-input
            v-model="prompt"
            type="textarea"
            :rows="5"
            :placeholder="$t('freeCreate.promptPlaceholder')"
            class="prompt-input"
          />
        </div>

        <div v-if="mode === 'video'" class="form-section">
          <div class="form-label">{{ $t('freeCreate.refImageLabel') }}</div>
          <div class="ref-image-zone" @click="triggerRefImageUpload" @dragover.prevent @drop.prevent="onRefImageDrop">
            <template v-if="refImageDataUrl">
              ...
              <div class="ref-actions">
                <el-button size="small" type="danger" plain @click.stop="clearRefImage">{{ $t('freeCreate.removeRefImage') }}</el-button>
              </div>
            </template>
            <template v-else>
              <el-icon class="upload-icon"><Picture /></el-icon>
              <div class="upload-tip">{{ $t('freeCreate.uploadRefImageTip') }}</div>
            </template>
          </div>
        ...
        <div class="form-section form-row">
          <div class="form-item">
            <div class="form-label">{{ $t('freeCreate.styleLabel') }}</div>
            <el-input v-model="style" :placeholder="$t('freeCreate.stylePlaceholder')" />
          </div>
          <div v-if="mode === 'image'" class="form-item">
            <div class="form-label">{{ $t('freeCreate.ratioLabel') }}</div>
          ...
          <div v-if="mode === 'video'" class="form-item">
            <div class="form-label">{{ $t('freeCreate.durationLabel') }}</div>
            <el-select v-model="duration">
              <el-option :label="$t('freeCreate.duration3s')" :value="3" />
              <el-option :label="$t('freeCreate.duration5s')" :value="5" />
              <el-option :label="$t('freeCreate.duration8s')" :value="8" />
              <el-option :label="$t('freeCreate.duration10s')" :value="10" />
            </el-select>
          </div>
        </div>
        ...
        <el-button
          ...
          @click="generate"
        >
          {{ generating ? $t('freeCreate.generating') : (mode === 'image' ? $t('freeCreate.generateImage') : $t('freeCreate.generateVideo')) }}
        </el-button>
      </div>
      ...
      <div class="result-panel">
        <div class="result-header">
          <span class="result-title">{{ $t('freeCreate.resultTitle') }}</span>
          <el-button v-if="results.length > 0" size="small" plain @click="clearResults">{{ $t('freeCreate.clearResults') }}</el-button>
        </div>

        <div v-if="results.length === 0 && !generating" class="empty-result">
          <el-icon class="empty-icon"><MagicStick /></el-icon>
          <p>{{ $t('freeCreate.emptyResult') }}</p>
        </div>

        <div v-if="generating" class="generating-tip">
          <el-icon class="is-loading"><Loading /></el-icon>
          <span>{{ $t('freeCreate.generatingTip') }}</span>
        </div>
          ...
          <div v-for="(item, idx) in results" :key="idx" class="result-item">
            <div class="result-media">
              ...
              <div v-else-if="item.status === 'pending' || item.status === 'processing'" class="media-loading">
                <el-icon class="is-loading"><Loading /></el-icon>
                <span>{{ item.status === 'processing' ? $t('freeCreate.generating') : $t('freeCreate.queuing') }}</span>
              </div>
              <div v-else-if="item.status === 'failed'" class="media-error">
                <el-icon><CircleClose /></el-icon>
                <span>{{ item.error || $t('freeCreate.generateFailed') }}</span>
              </div>
            </div>
            <div class="result-meta">
              <span class="result-prompt">{{ item.prompt }}</span>
              <div class="result-actions">
                <el-button v-if="item.url" size="small" plain @click="downloadItem(item)">{{ $t('freeCreate.download') }}</el-button>
              </div>
            </div>
          </div>
        </div>
  ...
  try {
    if (mode.value === 'image') {
      ...
      if (res?.task_id) {
        ...
        newItem.error = t('freeCreate.submitFailed')
      }
    }
  } catch (e) {
    newItem.status = 'failed'
    newItem.error = e.message || t('freeCreate.generateFailed')
  ...
  while (Date.now() - start < maxMs) {
    ...
    try {
      ...
      if (res.status === 'failed') {
        item.status = 'failed'
        item.error = res.error || t('freeCreate.generateFailed')
        return
      }
    } catch (_) {}
  }
  item.status = 'failed'
  item.error = t('freeCreate.timeout')
}
