...
export function useCharacters(deps) {
  ...
  // ── 衣橱（服装造型）状态 ─────────────────────────────
  const charEditTab = ref('info')
  const charOutfits = ref([])            // 当前编辑角色的造型列表
  const charOutfitsLoading = ref(false)
  const outfitPanelOpen = ref(false)     // 新增/编辑表单是否展开
  ...
  async function loadCharOutfits() {
    const form = editCharacterForm.value
    if (!form?.id) { charOutfits.value = []; return }
    charOutfitsLoading.value = true
    try {
      charOutfits.value = await outfitAPI.listByCharacter(form.id)
    } catch (e) {
      console.warn('[衣橱] 加载造型失败', e.message)
    } finally {
      charOutfitsLoading.value = false
    }
  }

  function stopOutfitViewPoll() {
    if (outfitViewPollTimer) {
      clearInterval(outfitViewPollTimer)
      outfitViewPollTimer = null
    }
  }

  /** 打开新增造型表单 */
  function onOutfitCreate() {
    outfitPanelOpen.value = true
    outfitForm.value = {
      character_id: editCharacterForm.value?.id,
      name: '',
      description: '',
      hair_style: '',
      face_shape: '',
  ...
  function onOutfitCancel() {
    outfitPanelOpen.value = false
    outfitForm.value = null
  }

  async function onOutfitSave() {
    const form = outfitForm.value
    if (!form?.character_id) return
    if (!form.name?.trim()) { ElMessage.warning('请填写造型名称'); return }
    outfitSaving.value = true
    try {
      if (form.id) {
        await outfitAPI.update(form.id, {
          name: form.name.trim(),
          description: form.description || undefined,
          hair_style: form.hair_style || undefined,
          face_shape: form.face_shape || undefined,
          body_type: form.body_type || undefined,
          skin_tone: form.skin_tone || undefined,
          signature_accessory: form.signature_accessory || undefined,
        })
        ElMessage.success('造型已更新')
      } else {
        await outfitAPI.create(form.character_id, {
          name: form.name.trim(),
          description: form.description || undefined,
          hair_style: form.hair_style || undefined,
          face_shape: form.face_shape || undefined,
      ...
      outfitPanelOpen.value = false
      outfitForm.value = null
      await loadCharOutfits()
    } catch (e) {
      ElMessage.error(e.message || '保存造型失败')
    } finally {
      outfitSaving.value = false
    }
  }

  function onOutfitEdit(o) {
    outfitPanelOpen.value = true
    outfitForm.value = {
      id: o.id,
      ...
      face_shape: o.face_shape || '',
      body_type: o.body_type || '',
      skin_tone: o.skin_tone || '',
      signature_accessory: o.signature_accessory || '',
    }
  }

  async function onOutfitSetDefault(o) {
    try {
      await outfitAPI.setDefault(o.character_id, o.id)
      ElMessage.success('已设为默认造型')
      await loadCharOutfits()
    } catch (e) {
  ...
  async function onOutfitDelete(o) {
    try {
      await ElMessageBox.confirm(`确定删除造型「${o.name}」？`, '删除造型', { type: 'warning' })
    } catch { return }
    try {
      await outfitAPI.remove(o.id)
  ...
  /** 从造型描述提炼特征锚点（发型/脸型/体型/肤色/配饰） */
  async function onOutfitExtractAnchors() {
    const form = outfitForm.value
    if (!form?.id) return
    outfitAnchorsExtracting.value = true
    try {
      const res = await outfitAPI.extractAnchors(form.id, form.description)
      const upd = res?.anchors
      if (upd) {
        if (upd.hair_style) form.hair_style = upd.hair_style
        if (upd.face_shape) form.face_shape = upd.face_shape
        if (upd.body_type) form.body_type = upd.body_type
        if (upd.skin_tone) form.skin_tone = upd.skin_tone
        if (upd.signature_accessory) form.signature_accessory = upd.signature_accessory
        ElMessage.success('特征已提炼并填入')
      } else {
        ElMessage.warning(res?.message || '未能提炼出特征')
      }
    } catch (e) {
      ElMessage.error(e.message || '提炼特征失败')
    } finally {
      outfitAnchorsExtracting.value = false
    }
  }

  /** 提交三视图生成任务，并轮询回写图片路径 */
  async function onOutfitGenerateViews(o) {
    try {
      const res = await outfitAPI.generateViews(o.id, {})
      const tasks = res?.tasks || []
      if (!tasks.length) { ElMessage.warning('未返回生成任务'); return }
      outfitViewGenIds.add(o.id)
      // 立即轮询一次，随后定时轮询
      const trySync = async () => {
        try {
          const st = await outfitAPI.generateViewsStatus(o.id)
          if (st?.status === 'completed' || st?.updated_fields?.length > 0) {
            stopOutfitViewPoll()
            outfitViewGenIds.delete(o.id)
            await loadCharOutfits()
      ...
      await trySync()
      stopOutfitViewPoll()
      outfitViewPollTimer = setInterval(trySync, 4000)
      ElMessage.success('三视图生成任务已提交，生成中…')
    } catch (e) {
      ElMessage.error(e.message || '提交生成失败')
    }
  }

  /** 造型视图图 URL（兼容 local_path / URL，与 assetImageUrl 语义一致） */
  function outfitViewSrc(o, view) {
    const map = { front: 'front_image_path', side: 'side_image_path', back: 'back_image_path' }
    const raw = o?.[map[view]]
    if (!raw) return ''
  ...
  function editCharacter(char) {
    ...
    // 打开"服装造型"页签时加载造型（这里预加载，切页签立即有数据）
    charEditTab.value = 'info'
    outfitPanelOpen.value = false
    outfitForm.value = null
    loadCharOutfits().catch(() => {})
    showEditCharacter.value = true
